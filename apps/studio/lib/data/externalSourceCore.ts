...
function mapTmdbType(mediaType: string): Film['type'] {
  return mediaType === 'tv' ? 'tv_series' : 'movie';
}

function mapOmdbType(type: string): Film['type'] {
  const normalized = type.toLowerCase();
  return normalized === 'series' || normalized === 'episode' ? 'tv_series' : 'movie';
}
...
function slugFrom(value: string): string {
  return value.toLowerCase().replace(/\s+/g, '-').replace(/[^a-z0-9-]/gi, '');
}

function getTmdbKey(): string {
  const key = process.env.TMDB_API_KEY || process.env.NEXT_PUBLIC_TMDB_API_KEY;
  if (!key) throw new Error('TMDB key missing. Add NEXT_PUBLIC_TMDB_API_KEY or TMDB_API_KEY to .env.');
  return key;
}

function getOmdbKey(): string {
  const key = process.env.OMDB_API_KEY || process.env.NEXT_PUBLIC_OMDB_API_KEY;
  if (!key) throw new Error('OMDb key missing. Add NEXT_PUBLIC_OMDB_API_KEY or OMDB_API_KEY to .env.');
  return key;
}
...
export async function searchExternalFilmsServer(query: string, source: ExternalSourceName): Promise<ExternalSearchResult[]> {
  ...
  if (source === 'IMDb ID') {
    ...
    if (!/^tt\d{7,8}$/u.test(cleanImdb)) throw new Error('IMDb ID must look like tt1234567');
  ...
  const raw = await readJson<{
    docs?: Array<{ key: string; title: string; first_publish_year?: number; cover_i?: number }>;
...
export async function fetchExternalDetailsServer(result: ExternalSearchResult): Promise<Partial<Film>> {
  if (result.source === 'TMDB') {
    ...
    const mediaType = result.type === 'tv_series' ? 'tv' : 'movie';
    const response = await fetch(`https://api.themoviedb.org/3/${mediaType}/${idNum}?api_key=${tmdbKey}`, {
      cache: 'no-store',
    });
    const raw = await readJson<{
      title?: string;
    ...
    return {
      ...
      runtimeMinutes: raw.runtime ?? raw.episode_run_time?.[0],
      ...
      tmdbId: raw.id ? String(raw.id) : undefined,
    };
  }
  ...
  if (result.source === 'OMDb' || result.source === 'IMDb ID') {
    ...
    const raw = await readJson<Record<string, string>>(response, 'OMDb');
    const title = raw.Title || result.title;
