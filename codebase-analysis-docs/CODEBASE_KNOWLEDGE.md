...
- Desktop home right rail renders `The Lobby` from `features/audio-rooms` above people suggestions. It is a UI-only, bounded audio-room summary contract with an honest empty state for signed-in users while no production room API, signaling, media transport, or presence service exists. Guests do not see the empty lobby or the people-suggestions card. The rail caps output at four rooms; populated summaries show one host, at most three speaker avatars, a topic, and a denormalized listener count. No fake live data or inert join/create action ships.
