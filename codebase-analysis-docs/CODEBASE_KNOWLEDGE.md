...
Generated from a direct repository inspection on 2026-06-23. Last refreshed for full content moderation backend on 2026-07-11, mobile shell navigation behavior on 2026-07-17, React Native email verification on 2026-07-24, and the signed-out web landing experience on 2026-07-25.
...
- `apps/mobile`: Expo/React Native iOS and Android workspace; Phase 2 launch, auth bootstrap, Welcome, and the complete five-step signup/verification path are implemented over the Phase 1 foundation. Canonical plan and continuation status live in `docs/react-native-mobile-development-plan.md`.
...
Remaining account and product flows remain planned. Future mobile sessions must read and maintain
...
Testing uses Jest 29.7 through `jest-expo` 57.0.2 and React Native Testing Library 14.0.1. Jest discovery is rooted at `src`, so ignored generated-native and local Expo cache artifacts cannot make test enumeration unbounded. Tests stay under `src/test`, outside the Expo Router route tree. Package commands separate non-watching local tests, explicit watch mode, and serialized CI coverage. Root `pnpm mobile:check` validates `@35mm/design-tokens`, `@35mm/api-client`, and `@35mm/mobile-ui`, then runs mobile typecheck, Expo lint, two-variant public config/profile validation, quality-harness contracts, native policy/isolated Prebuild checks, and tests as one CI-safe command. Current coverage includes 29 token invariants, 6 API-client cases, and 90 mobile app/UI cases spanning deterministic foundation gallery behavior, launch/bootstrap state, auth recovery, Welcome content/navigation/legal behavior, all five signup steps, draft migration/privacy/restoration, username debounce/cancellation/error recovery, volatile-secret process recreation, Clerk payload privacy/resume, code paste/autofill/cooldown/resend/change-email/expiry behavior, session-before-profile-completion ordering, verified completion retry/recovery with or without a retained SignUp resource, matching-user recovery routing, password visibility and password-manager metadata, duplicate-action blocking, 44-point targets, variant-specific Apple Sign-In capability, and recent-bundle auto-launch preference.
