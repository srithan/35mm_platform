...
Generated from a direct repository inspection on 2026-06-23. Last refreshed for full content moderation backend on 2026-07-11, mobile shell navigation behavior on 2026-07-17, React Native email verification on 2026-07-24, NSFW classification plus web presentation on 2026-07-25, the signed-out web landing redesign on 2026-09-03, React Native mixed posts/comments on 2026-09-06, the React Native authenticated shell plus notification/bookmark/list/profile surfaces on 2026-09-13, React Native password recovery on 2026-09-20, the retained SwiftUI plus React Native auth-flow refresh on 2026-09-20, retained SwiftUI home scroll chrome on 2026-09-21, retained SwiftUI Discover header tabs on 2026-09-21, retained SwiftUI feed refresh-state repair on 2026-09-22, retained SwiftUI post/profile navigation repair and PostCard author-route hardening on 2026-09-22, and retained SwiftUI shared feed renderer reuse on 2026-09-23.
...
- `apps/mobile`: Expo/React Native iOS and Android workspace; Phase 2 launch/auth/signup/password-Login/password-recovery, the authenticated five-tab shell/drawer map, production notification list/read controls, production bookmark folders/move/remove/search, production profile tabs/edit/connections/actions, production chat core, and a user-prioritized Phase 4 production mixed-post feed/video-composer/comment-reader slice are implemented over the Phase 1 foundation. Canonical plan and continuation status live in `docs/react-native-mobile-development-plan.md`.
...
The authenticated mobile product surface now includes a real cursor feed, recycler-backed mixed post cards, native versioned-rich-text rendering, PostCard More actions, cursor-paged post comments, signed Bunny playback, account autoplay/sound policy, eager resumable direct uploads, processing states, visibility, optimistic interactions, share, owner soft-delete, production notification read-state controls, production bookmark folders/move/remove/search, and production profile tabs, connections, media preview, edit profile, and social/moderation actions. Remaining account and product flows remain planned. Future mobile sessions must read and maintain
...
- `middleware.ts`: Clerk route protection. `/landing` redirects to `/`; guest-only auth pages (`/login`, `/signup`, `/forgot`, `/reset`, `/verify`) redirect authenticated sessions in middleware before page render. `isPublicContentPath()` additionally serves read-only content pages to signed-out visitors — profiles (`/:username`), profile tabs, post detail, discover, films, lists, list detail, `/film`, `/tv`, `/title`, `/person`, `/company`, and `/:role/:person-slug` — while app sections (chat, bookmarks, drafts, new, notifications, settings, 70mm, suggestions, contribute, festivals, communities, onboarding, and the authed root feed) stay protected. Guest browsing relies on the read APIs' existing `getOptionalAuthUser` support. Signed-out chrome is handled client-side: `features/auth/components/AuthPromptProvider` (`useAuthPrompt` → `promptLogin({ mode, title, message, next })`/`requireAuth` + `AuthModal`) is mounted in `ShellGrid`. `AuthModal` is a two-column centered dialog (left: bundled landing artwork plus contextual headline/message; right: `LoginForm`, `SignupForm`, or `VerifyEmailForm` one at a time so height follows the visible form) that collapses to the form column on mobile; `next` is a prop, sanitized to same-origin relative paths. `SiteHeader` Log in / Sign up open it in the matching mode (links keep `/login`/`/signup` hrefs as fallback); `MobileTabBar`/`MobileHeader` gate account-only nav through the prompt, and title-page watched/watchlist/review actions prompt instead of mutating for guests. The same `requireAuth` gate covers public-page account actions: profile follow/request/message, post like/repost/bookmark/quote, comment like/reply, poll vote, and list like/clone/create. Share stays available. Profile, post, and comment overflow menus hide signed-in actions (mute/block/report/bookmark/hide) for guests and keep only Share. Signed-out visitors also skip `HomeProfileCompletionSidebar`, the people-suggestions card, and the Letterboxd import card. The home/profile rail hides an empty Lobby for all viewers (`LobbyRoomsWidget` `hideWhenEmpty`) and hides signed-in people suggestions when loading finishes with no results.
- `app/page.tsx`: session-aware root. Signed-out visitors render `features/landing` as a restrained, single-field
  cinematic surface with a curated TMDB poster strip, centered product copy, one primary join action, and Discover/Films/Lists
  navigation. `LandingPosterStrip` continuously scrolls 116 distinct posters from 24 language groups (24 English-language films, four each for the other 23) using a linear CSS
  transform (one poster per 2 seconds, 232 seconds per sequence). Two identical sequences create an invisible loop seam;
  the duplicate is hidden from assistive technology. Pause/play is explicit; reduced-motion shows a static strip without
  the duplicate; intersection/document-visibility observers suspend hidden animation. This is UI-only state with no API
  polling, per-frame React updates, DB work, queue jobs, or new indexes at 1M+ DAU. DOM stays fixed at 232 images; only
  six initial posters have priority, remaining assets lazy-load with intersection-based loading one rail-width ahead
  (disabled for reduced motion) to avoid empty entering frames. The 116 distinct Next Image-optimized TMDB CDN URLs live in
  `features/landing/lib/landingPosters.ts`, with no runtime metadata requests. `landingPosterPolicy.json` gives English 24 slots, uniformly
  spaced every four or five posters, including 21 US-production slots and one each for AU/GB/NG. The other 23 languages
  keep four interleaved slots each. This gives Hollywood greater presence. The policy excludes all cast/crew film credits of Vijay
  (Joseph Vijay, TMDB person 91547), Ajith Kumar (148360), and Sivakarthikeyan (587982). IDs avoid excluding Vijay Sethupathi.
  Tamil pins Vaaranam Aayiram, 24, Jai Bhim, and Vishwanath & Sons at positions 2/31/60/89, requiring Suriya (85720) in
  verified cast credits. Pinned selections preserve per-language quotas and must pass every existing exclusion check;
  missing/ineligible pins abort regeneration. Tests protect these film identities and their spacing.
  `node apps/web/scripts/refresh-landing-posters.mjs` refreshes the selection offline using full TMDB film/person credits with
  bounded concurrency/pagination, refusing incomplete verification or quotas. `landingPosterAudit.json` is test-only evidence;
  curation tests enforce credit exclusions, per-language quotas, featured-film spacing, interleaving, country rotation, uniqueness, and manifest/audit parity;
  the auth modal keeps bundled artwork. Existing Clerk signup/login operations open in a centered two-column desktop modal and a responsive
  bottom action sheet on mobile web. Auth fields stay at 16px so iOS Safari does not zoom the page on focus.
  Password login on both the landing modal and `/login` continues Clerk
...
- Web UI rollout flags resolve through `apps/web/lib/config/uiFlags.ts`.
  `resolveUiRolloutConfig()` maps public env booleans into typed variants for
  desktop navigation, browse chrome/density, mobile tab bar presentation, and home composer entry. Components
  should consume those variants or derived constants instead of re-combining env
  flags locally; this preserves compile-time rollout behavior and gives future
  per-user A/B assignment one resolver seam. Client-only presentation variants
  add no API read/write, cache, worker job, schema, or index.
- `NEXT_PUBLIC_BROWSE_RAIL` is a public, compile-time web UI flag for the desktop browse rail layout. Default `true` enables the desktop left-rail browse menu, Home top-nav label, removed desktop Discover nav item, removed Discover/Films/Lists subnav, wider 864px Discover/Films/Lists center column with a capped 220px rail, one-column `/lists` list view, `/film`, `/tv`, `/title`, and `/person` detail pages spanning the browse center column plus the capped right rail column, and `/list/:id` pages spanning the 640px home center column plus the capped right rail column. Set it to `false` and restart/rebuild web to restore the older desktop browse/title/person/header/sidebar layout.
- `NEXT_PUBLIC_SIMPLIFIED_POST_COMPOSER_TRIGGER` is a public, compile-time web UI flag for the home feed composer trigger. Default `false` keeps the current trigger with composer chips and the Post affordance; `true` renders a stripped trigger with only the viewer avatar and first-name "What's on your mind?" prompt while preserving the same modal open behavior. It takes precedence over `NEXT_PUBLIC_INLINE_POST_COMPOSER` so the simplified trigger can roll out in environments using the inline composer. This client-only presentation flag adds no API read/write, cache, worker job, schema, or index.
- `NEXT_PUBLIC_MOBILE_TAB_BAR_TRADITIONAL` is a public, compile-time web UI flag for the mobile bottom tab bar. Default `false` keeps the modern floating pill; `true` renders the same tabs as a full-width, square-cornered bar flush to the viewport bottom while preserving the existing scroll-down hide and scroll-up show behavior. This client-only presentation flag adds no API read/write, cache, worker job, schema, or index.
- `TraditionalTabBarEnabled` is the retained SwiftUI iOS tab-bar flag, read from Info.plist, `TRADITIONAL_TAB_BAR_ENABLED`, or UserDefaults key `traditionalTabBarEnabled`. Default `true` renders an app-owned fixed full-width Home, Discover, Add, Activity, and Profile bar that avoids system `UITabBar` layout reservation; set it to `false` to restore the previous system Home/Create/Activity shell. Add opens the composer and returns to the previous durable tab. Reselecting an active Home, Discover, Activity, or Profile item clears only that tab's typed `NavigationStack` path, so Home returns from Bookmarks or another pushed destination to Feed while inactive tabs retain independent navigation history. Every traditional item hides its visible text while preserving its accessible label; Home, Discover, Add, and Activity use selected filled plus inactive outline SVG variants at a consistent 24pt glyph size, and Profile renders the current user's avatar or initials fallback instead of a generic person icon. The SVG tab assets are bundled as template-rendered vector image sets so current theme tokens still own active and inactive colors. Home feed down-scroll hides the header and bottom chrome through a shared `UIScrollView` pan observer; returning near top or scrolling upward restores both, matching mobile web and honoring Reduce Motion. Discover, Activity, Bookmarks, and Profile reuse that observer with header-only behavior: upward swipe/deeper scroll hides their page/header chrome, downward swipe/back-scroll or top lock reveals it, and the bottom tab bar stays visible. The Home header and traditional tab bar slide/fade in independently animated overlay layers while the Home scrolling surface extends through the bottom safe area. Both feed insets stay fixed across traditional-bar visibility changes, including bottom clearance for the bar and safe area; this preserves last-row reachability and prevents UIKit from shifting the feed when its scroll range changes at the tail. The traditional Home visibility callback does not animate the feed subtree; only the header and tab overlays receive the chrome animation. Reduce Motion disables those animations. This local chrome flag adds no API read/write, cache, worker job, schema, or index.
- `NEXT_PUBLIC_POST_MEDIA_CAROUSEL` is a public, compile-time web UI flag for PostCard image presentation. Default `false` keeps the existing grid for two to four images and the existing carousel for five or more images. `true` renders multi-image PostCard photos as a horizontally swipeable peek carousel with a left-aligned position indicator above the photos. React Native uses matching opt-in flag `EXPO_PUBLIC_POST_MEDIA_CAROUSEL`; the retained SwiftUI app uses `PostMediaCarouselEnabled`, environment `POST_MEDIA_CAROUSEL_ENABLED`, or UserDefaults `postMediaCarouselEnabled`. Retained SwiftUI defaults the flag on in Info.plist, applies it only to posts with more than two images so one- and two-image posts keep the grid, and can revert to grid with `POST_MEDIA_CAROUSEL_ENABLED=0` or UserDefaults `postMediaCarouselEnabled=false`. PostComposer quoted-post previews continue to use the existing quote grid. These client-only presentation flags reuse already-loaded post media and add no API read/write, cache, worker job, schema, or index.
- `NEXT_PUBLIC_FOCUSED_NAVIGATION` is a public, compile-time desktop UI flag. Default `false` keeps `SiteHeader`; `true` uses `FocusedNavigationSidebar` as desktop's single fixed left navigation and suppresses duplicate Browse rail menu while retaining profile-setup widget. Focused home removes the former empty left grid track, leaving content and right-space tracks next to fixed navigation. Discover, Films, Lists, movie-title, and people routes use one centered content column without auxiliary rails, capped at 1240px. Post-detail's 640px column offsets by half fixed rail width, preserving full-viewport centering. Discover, Films, and Lists share a desktop-only top navigation strip, while mobile navigation stays unchanged. It includes brand, Home, Discover, Watch, Messages, Activity, Profile, composer entry, and bottom `More`. `More` reuses all existing `ProfileMenu` actions/subviews, including appearance and privacy. Existing mobile header/tab navigation remains unchanged. This client-only presentation change adds no API read/write, cache, worker job, schema, or index; at 1M+ DAU it adds zero incremental backend volume.
- `NEXT_PUBLIC_FOCUSED_NAVIGATION_CENTERED` independently controls focused-sidebar primary-nav alignment. Default `false` retains top alignment; `true` vertically centers Home through Create while preserving top brand and bottom account actions. This client-only presentation flag adds no API read/write, cache, worker job, schema, or index.
...
- `features/films`: `/films` is the searchable/filterable catalog browse surface. List view renders rounded title rows with director, metadata chips, and an authenticated Add-to-list dropdown backed by existing profile list reads and `POST /v1/lists/:listId/entries`; guests get the shared auth prompt.
...
- `ThirtyFiveMM/Features/Intro/IntroView.swift` and `WelcomeHeroView.swift`: fixed-light signed-out welcome with three staggered columns of nine locally bundled movie posters, white bottom fade, bare top wordmark, “Your cinema. Your people.” headline and explicit cinema social-network subtitle, black Sign up and gray Log in capsules, and Terms/Privacy links. Standard layouts remain fixed; enlarged text and compact landscape can scroll. Poster columns loop at different speeds on a visibility/scene-gated 30 Hz timeline, with static Reduce Motion output. Flexible hero height keeps actions near the bottom safe area. Artwork provenance lives in `Resources/WelcomePosters.md`; no runtime image fetch or authenticated API read is added before the existing Clerk-backed auth routes.
- `ThirtyFiveMM/Features/Auth/SignInView.swift`, `SignUpView.swift`, and `AuthScaffold.swift`: prompt-first retained SwiftUI auth forms without the old poster hero on login/signup. Signup is top-aligned and uses progress dots plus one requested field per visual step: name, username, email, password, private DOB, and verification. Login is also top-aligned and uses email-or-username plus password with password reveal. The iOS form surface uses plain white backgrounds, black/gray fields, and standard SF system typography; beige/brown auth palettes, centered form blocks, and rounded/serif display fonts are deliberately excluded from these screens.
- `ThirtyFiveMM/App/AppConstants.swift`: API and web base URLs, Clerk publishable key, optional Ably API key, and retained SwiftUI UI flags loaded from `ThirtyFiveMM.xcconfig`, Info.plist, environment, or UserDefaults. The configurable web origin lets native Discover share the Next `/api/tmdb` proxy in local and deployed environments.
- `ThirtyFiveMM/Core/Auth/AuthManager.swift` and `ThirtyFiveMM/App/RootView.swift`: Clerk session restoration, launch-screen-matching wordmark handoff during bootstrap, API bootstrap/onboarding gate, signup DOB persistence through the existing owner-only profile update path, and retry/sign-out recovery when Clerk or app bootstrap is unavailable. The handoff uses no visible progress spinner so startup does not become a second branded loading screen. Startup does not route to signed-out until Clerk has finished loading; a Clerk restore timeout remains on the recovery surface so app restarts or appearance changes cannot falsely discard a persisted session.
- `ThirtyFiveMM/Core/DesignSystem.swift`: app-wide design tokens — brand accent `#c2473a` (also set in `AccentColor.colorset`), like/repost/bookmark action colors, 4pt spacing scale, corner-radius scale, canonical avatar sizes, and semantic Dynamic Type font aliases (`.appWordmark`, `.appScreenTitle`, `.appAuthorName`, `.appMetadata`, etc.). Shell chrome (tab bar, header, sidebar, composer), feed identity/timestamps, notifications, settings, and chat headers use these tokens or system text styles; the traditional bottom tab bar uses template-rendered bundled filled/outline SVG assets for Home/Search/Compose/Activity plus the current user's avatar for Profile, while the system fallback keeps SF Symbols. All non-profile traditional tab glyphs use the same 24pt size. Signed-out welcome can keep custom hero artwork, but auth form screens stay neutral and system-typographic.
- `ThirtyFiveMM/Core/Theme.swift`: web-parity color themes (`auto | light | dark | letterboxd | matinee | matrix | oppenheimer-bw | barbie`) plus accent overrides. `ThemeManager` is local authority (UserDefaults); settings GET only hydrates when no persisted theme exists. `SettingsViewModel` lives on `AppEnvironment` so theme-driven view recreation cannot re-GET and flash a stale server theme; settings sections push via `AppRoute.settingsSection` on the tab stack path so theme rebuilds cannot pop Appearance. Switches snap `UIWindow.overrideUserInterfaceStyle` + opaque chrome (no `.preferredColorScheme`) without force-loading hidden UIKit controller views. Appearance saves are optimistic and merge over stale PATCH echoes. High-traffic surfaces, bottom action sheets, and profile header backgrounds read `@Environment(\.theme)` instead of system `Color(.background)`/hard-coded light-dark palettes.
...
- `ThirtyFiveMM/Core/BottomActionSheet.swift`: shared action-sheet surface plus Boolean/item presentation modifiers. Surface mirrors mobile web's `PortalDropdown` bottom sheet through its title-free 32pt shell, neutral 38% dark backdrop, theme-owned shell/groups/text/destructive/pressed/divider colors, 22pt grouped cards, 58pt rows, inset dividers, safe-area spacing, and 80pt drag-to-dismiss threshold. Native backdrop uses direct translucency instead of SwiftUI light material, which can become an opaque white wash inside transparent covers. Presenter suppresses system cover choreography, then drives backdrop fade and bottom-panel movement in one 300 ms smooth transaction for synchronized entry/exit. Reduce Motion uses an immediate path. Feed, comments, image viewer, bookmarks, profiles, titles, reviews, and credits use this presenter.
...
- `ThirtyFiveMM/Core/Models/Catalog.swift` and `ThirtyFiveMM/Features/Discover`: native catalog/TMDB discovery DTOs and service boundaries, debounced multi-search, and web-equivalent popular hero, streaming-provider, trending, ranked, current-release, popular, mood, TV, and Now Playing shelves. The root Discover tab now uses the shared `AppHeader`/`HeaderTabBar` presentation from Notifications with Discover, Lists, and Films sections; Lists reads existing public `/v1/lists?sort=popular` cursor pages, and Films reads existing `/v1/catalog/titles?type=movie` cursor pages before navigating by canonical catalog title ID. Discovery reads reuse web's cached/rate-limited `/api/tmdb` proxy; selection performs an indexed external-ID lookup through `/v1/catalog/titles` and uses only the returned canonical catalog ID for native navigation. Provider changes reload only the streaming shelf.
...
- `ThirtyFiveMM/Features/Feed/FeedCollectionView.swift`, `FeedViewModel.swift`, `PostDetailViewModel.swift`, and `PostCard.swift`: native home/detail post interaction state with optimistic likes/reposts/bookmarks/poll votes, comment navigation, rich text, media grids or opt-in `PostMediaCarousel.swift` 3+-image peek carousel, link previews, web-aligned poll result UI, and cinematic film-log cards whose poster-derived glow uses a bounded on-device color cache. Home and Profile Posts/Reposts share the loaded-row `UICollectionView` bridge with compositional self-sizing rows, diffable item identity keyed only by stable post ID, and shape-specific reuse identifiers for text, review/log, media, and poll cards. Profile embeds that same renderer with scrolling disabled and measured-height reporting so `ProfileTabPager` remains the horizontal paging owner. Content-only updates compare a render fingerprint, not `FeedPost` equality, and flow through `snapshot.reconfigureItems`, so optimistic counters/flags and poll state update in place even though `FeedPost ==` is ID-only. The current cell bodies host SwiftUI `PostCard` through `UIHostingConfiguration`; the registration boundary is the seam for later UIKit bodies for hot text/media cards. The first configuration loads the UIKit view before applying the whole page. Serialized, nonanimated snapshots skip unchanged updates and reconcile changed surviving IDs during insertion; reuse-shape changes reload cells. Hosted content uses required-width/natural-height fitting and post-scoped SwiftUI identity, with UIKit owning measured sizes instead of a write-only height cache. Hosted cells expose zero safe-area insets: viewport clearance stays on the collection, preventing row heights and spacing from changing as cells cross screen edges during scrolling. An idle visible-post anchor survives later intrinsic-size passes and releases for user scrolling; initial overlay insets and inset-normalized active-pan chrome tracking prevent top-of-feed jumps. Embedded Profile height reports are deferred and deduplicated. Prefetch resolves IDs from the committed snapshot, with refresh/apply guards and one-request-per-tail pagination five rows before the loaded tail. This adds no backend reads/writes or indexes at 10M DAU and retains existing 20-post cursor reads, hybrid fan-out/cache, rate-limit, soft-delete, and async-counter contracts. `PostMediaImageButton.swift` installs a non-interactive UIKit anchor in each actionable image tile. `PostImageViewerPresenter.swift` presents the SwiftUI viewer through a custom `UIHostingController`; the transition snapshots the processed Kingfisher memory-cache image from its exact window frame and lands on a measured viewer-image anchor while covering the live image. The full-screen pager owns safe-area geometry. Shorter full-width images are vertically centered between the reserved top and bottom controls through the shared image-frame calculation; oversized images retain their existing top inset and full width. Downward dragging transforms only an image snapshot in stationary container coordinates while the fixed viewer chrome/backdrop fade rapidly. A cover prevents the live image from duplicating the snapshot. Cancellation/upward reversal restores both image and viewer before removing transient layers; successful dismissal hands the snapshot frame to the closing hero without carrying controls or black padding. Dynamic gesture failure priority keeps nested page-scroll pans from stealing vertical dismissal, including lazily created pages. The original page reverses to its weak live source anchor; another page uses its current cached image and a downward offscreen target. Viewer chrome uses compact controls, toggles on image/backdrop tap, and pages support bounded 1x–4x pinch zoom; paging and downward dismissal pause while zoomed. Missing cached imagery and Reduce Motion use an opacity fallback. Comment/quote navigation waits for dismissal completion. Feed cards clamp long bodies while detail cards render them completely; TipTap paragraphs use one line break. `LinkPreviewCard`, `URLVideoPreview`, and `VideoURLPreview` give native post/comment URLs web-parity image-first, text-only, and provider-video treatments; per-body detection is bounded, de-duplicated, and performs no server read beyond existing remote thumbnail delivery. Post detail keeps the reply trigger/composer inline above cursor-paged comments, and comment cards expose compact like/reply-count/reply actions over existing interaction endpoints. Post and comment identity stacks share compact role/context/films-logged presentation; their avatar/name/username controls emit typed `AppRoute.profile` destinations to the shared native profile screen. Feed/profile/bookmark/notification post opens now push typed `AppRoute.post(PostDestination)` onto the owning tab path through `AppRouteNavigator`; already-loaded posts travel as initial detail data, and ID-only quote links still use the bounded remote detail read. This avoids stale local `navigationDestination(item:)` post bindings re-pushing detail above a profile route. Shared `FeedAuthorIdentityLabel`/`FeedTimestampLabel` views use `DesignSystem` Dynamic Type tokens (bold subheadline name, regular subheadline handle/timestamp on one line, Twitter-style), feed bodies use 15pt `.appBody` with 3pt line spacing (17pt `.appBodyLarge` on detail), while `feedRelativeShort` matches web's `now`/minute/hour/unbounded-day values without switching older feed items to calendar dates. `Core/ShareModal.swift` replaces the generic activity controller for the branded post/profile share path and reproduces mobile web's preview, 5x2 destination grid, copy control, 32pt sheet geometry, and coordinated backdrop motion using already-loaded content only. The regular-height shell has no height frame and hugs content directly; bounded scrolling exists only inside the compact-height body. The sheet is precomposed offscreen and animates as one container, with an eager fixed grid preventing staged child insertion.
- `ThirtyFiveMM/Features/Feed/FeedServicing.swift`: small retained SwiftUI feed service boundary used by `FeedViewModel` and focused tests. Initial loads stage short first pages (fewer than six unique posts) and at most one 20-item continuation before publishing, so a retained-to-cold-history handoff does not briefly expose a lone post above blank space. Full/exhausted pages publish immediately; continuation failure preserves readable rows, an explicit error, and the retry cursor; cancellation drops staged rows. Initial read budget is two requests maximum (one for ordinary pages), not an unbounded fill. Count/stage-only OSLog diagnostics exclude private payloads and cursors. Later refresh/revalidation merges the fresh first page over existing loaded rows, preserving already visible feed content across idle freshness checks, tab switches, and post-detail returns when the first-page response is short or transient. This adds no API route, database read/write shape, cache entry, worker job, schema, or index.
- Native feed/profile/bookmark `PostCard` instances use a full-card background `Button` for non-control detail navigation. Author avatar and full name/username/role clusters use explicit typed-profile buttons with full 44-point avatar hit areas; media, quote, overflow, poll, and action-bar controls also remain separate hit targets. Same-profile author clusters render inert, and an exact route already at the top of the owning tab path is not appended again. Detail cards omit the background button.
- Profile Posts/Reposts pass current profile identity through the shared collection renderer. Same-user avatar/name controls render without a navigation action, preventing duplicate profile pushes; different-author controls and non-profile cards keep typed profile routing.
...
- `ThirtyFiveMM/Features/Profile`: native mobile-web-aligned profile screen with cover/avatar identity, accessible full-screen avatar/cover previews through shared `Core/ImageViewerView.swift`, a role/context identity chip separate from denormalized inline counts, web-matched adaptive regular/strong header-action borders, and Posts/Reposts/Diary/Lists/Stats content. Horizontal paging keeps the stable outer vertical scroll: only the settled page participates in layout, while at most one destination page is mounted as a clipped overlay during drag/tap transition. `ProfilePagingPanGesture` uses a bounded `UIPanGestureRecognizer` bridge to confirm a 1.15 horizontal/vertical velocity ratio before paging and cancel a pending child-content tap only after that confirmation. Vertical scrolling remains simultaneous, nested horizontal shelves retain local priority, the system edge-back gesture wins, and VoiceOver disables touch paging while leaving the tab buttons available. Pushed profile pages opened from post/profile identity links suppress the regular app header, extend the cover/loading skeleton behind the status bar, keep the over-cover Back control in the compact header's left slot, blur the cover into a top header during scroll, fade the profile display name plus films-logged count into that header, and pin the tab bar under the compact cover header once profile identity content scrolls beneath it; the root Profile tab keeps shell header behavior. Translation drives LTR/RTL page offsets, underline position, label reveal/collapse, and subtle icon emphasis from one progress value. Settled active tabs always show icon plus label; inactive tabs show icons only. Drag and tap transitions interpolate outgoing and incoming tab chrome instead of snapping after page settlement; Reduce Motion commits directly. Posts/Reposts render through the shared `FeedCollectionView` instead of a profile-local `LazyVStack`, while Diary/Lists/Stats stay on their existing tab bodies. Loaded payloads remain in `ProfileViewModel`, but offscreen feed view trees do not stay mounted; destination reads begin on first presentation, and active-gated lazy pagination prevents previews from draining cursors. Independent 20-item post/repost/list cursor streams, lazy repost/list and cached-stats reads, optimistic post actions, app-standard bottom sheets for overflow/unfollow/block actions, share/copy actions, and validated edit-profile/media flows continue over existing REST endpoints. Reposts calls the existing profile feed route with `kind=reposts`; server-side filtering retains privacy/moderation enforcement and uses `posts_user_repost_created_at_id_idx` for user/activity cursor order. No backend shape, write, cache, queue, rate-limit, soft-delete, schema, or index work is added. Profile media presentation disables system cover choreography, captures the tapped avatar/cover frame, fades into a translucent black viewer, springs circular profile photos or fitted rectangular covers outward from the source, animates footer/close chrome, and uses a short fade under Reduce Motion. Preview delivery uses existing Kingfisher caching and bounded retry behavior.
- Pushed SwiftUI profiles reserve a 50pt cover header below the captured status-bar inset. Back/title/actions share that header; one pinned section tab bar sits underneath it. Collapse geometry uses the actual container width, and refresh scroll targeting includes the header inset. No backend traffic or contract changes. Retained SwiftUI profile collapse progress observes the outer native UIScrollView contentOffset and adjustedContentInset through the existing refresh-control bridge. Updates are coalesced on the main queue and observers detach with the view; SwiftUI geometry preferences no longer control header opacity. The blurred compact cover has an opaque backing; header and tab geometry remain unchanged. Profile name and share/more source frames are measured in stable content coordinates; each compact counterpart reveals as its source crosses the header edge. Cover blur and fades use scroll-driven smoothstep easing; Reduce Motion suppresses added vertical reveals.
- `AppSessionViewModels` now retains notification pager models and username-keyed `ProfileViewModel`s for the active authenticated user, in addition to feed/bookmarks/chat models. Activity/Profile tab switches, sidebar returns, and pushed profile destinations reuse loaded cursor/detail state instead of rebuilding loaders on every visit. Account changes clear the cache; explicit refresh/retry/read-state synchronization still uses existing bounded endpoints.
...
- `ThirtyFiveMMTests/ProfileFeatureTests.swift`: profile/stats/list fixture decoding, edit validation and explicit-null mutation encoding, cursor deduplication, signed LTR/RTL pager progress, adjacent-page and settlement behavior, lazy tab request deduplication, shared feed-renderer diffable reconfiguration under profile data, and optimistic interaction rollback.
...
Phase 2 launch wiring adds SDK-matched `expo-splash-screen` as an explicit CNG plugin/native module. Both generated platforms use a device-adaptive OS launch surface: white/black wordmark in light mode, black/white wordmark in dark mode, using the retained SwiftUI artwork. The root prevents automatic dismissal until the first React layout and hides the native surface once; the existing Clerk/query/font waits now render the same 151-by-56-point wordmark and device-adaptive background, with no artificial delay, motion, remote media, API request, or personalized state. Exact plugin/autolinking policy and isolated two-variant Prebuild cover the native impact, so the change requires new binaries rather than a JavaScript-only OTA.
...
The Phase 2 signed-out destination matches the retained SwiftUI Intro: device-adaptive colors, a bare wordmark, nine local posters in three Reanimated columns with fades, italic cinema copy, and black/gray account actions. Animation is bounded, background-paused, and static under Reduce Motion; compact/large-text layouts can scroll. It performs no API or remote-media read before an account action, routes to canonical signup-name and Login paths, and limits legal opening to fixed 35mm HTTPS URLs with accessible failure feedback.
...
The Phase 2 `/login` route is a device-adaptive Clerk password flow for username or email. It stores password and verification code only in component memory, blocks duplicate actions, maps credential errors without revealing account existence, requires a created Clerk session ID before activation, and routes successful sessions through the root bootstrap gate. Forward-compatible `needs_second_factor` and `needs_client_trust` states support only Clerk-advertised email-code factors with a 30-second resend cooldown; unsupported factor states remain explicit. Current Clerk configuration has no second factor. Apple and Google are enabled at the Clerk instance but remain separate native-provider work because the development entitlement/configuration is incomplete.

The Phase 2 password-recovery slice adds `/password/forgot`, `/password/verify`, `/password/reset`, and `/password/complete` over `features/auth/password`. It uses Clerk's enabled `reset_password_email_code` first factor, keeps reset codes and new passwords in component memory, allows only non-secret email/safe-target route state, applies resend cooldowns and privacy-safe errors, validates new-password confirmation locally, and either activates Clerk's returned session or returns to Login after completion. This is a Clerk-only auth flow with no 35mm API, database, cache, queue, worker, schema, migration, or index change.

`features/auth/signup` owns production `/signup/name`, `/signup/username`, `/signup/email`, `/signup/password`, `/signup/dob`, and `/signup/verify` routes plus one shared device-adaptive, top-aligned step scaffold for safe-area/keyboard handling, large-text layout, accessible six-step progress, and safe back behavior. Signup form steps intentionally have no cinematic hero. Name collects only full/display name; Username collects lowercase username with debounced availability and explicit retry; Email supplies normalized email keyboard, autofill, return-key, error-announcement, and duplicate-action semantics. Password supplies one password field with an 8-character minimum, accessible visibility control, secure entry, password-manager/autofill metadata, and iOS password rules. DOB supplies a formatted date control and shared virtualized wheel sheet with Done, locale ordering, leap-day clamping, future-date bounds, timezone-free canonical `YYYY-MM-DD` validation, impossible/future-date rejection, back/process restoration, safe Clerk errors, and duplicate-action blocking. Verification supplies one-time-code paste/autofill, a persistent 30-second resend cooldown, safe incorrect/expired/throttled recovery, and Clerk-authoritative change-email behavior. Versioned Zustand/AsyncStorage draft schema 4 persists bounded name, username, email, DOB, code-send time, and matching verified Clerk user ID while passwords, codes, session tokens, Clerk resources, and server responses remain excluded. Clerk's returned `complete` status and created IDs are required; the session activates before protected `/v1/me`, naturally idempotent `PATCH /v1/profiles/me`, and `/v1/me/onboarding-status` calls. Exact DOB persistence is runtime-validated, failures keep an explicit retry state across process recreation, success clears the draft and returns through the root onboarding router, and only a matching signed-in user can enter recovery. Shared `@35mm/validators/date-of-birth` enforces real calendar/future-date checks on mobile and API, while existing owner-only projection, profile rate limit, unique `profiles.user_id` path, and public DOB redaction remain. This adds no API route, DB schema, Redis/cache, queue/worker job, or index.

`APP_VARIANT` is mandatory and accepts only `development` or `preview`. Development identity is `35mm Dev`, `thirtyfivemm-dev`, and `com.thirtyfivemm.mobile.dev`; preview identity is `35mm Preview`, `thirtyfivemm-preview`, and `com.thirtyfivemm.mobile.preview`; SwiftUI remains `com.35mm.app`. Internal EAS profiles select matching EAS environments and variant values. Expo Dev Client belongs to development, uses launcher mode to disable automatic recent-bundle launch, and retains its generated URL scheme; physical-iOS evidence proves that setting is not a null-bundle startup fix. Preview disables that scheme and does not inherit launcher behavior. Clerk's supported plugin option disables Sign in with Apple only for development so an Apple Personal Team can provision it; preview retains the entitlement for paid-team validation. Public `extra.appleSignInEnabled` exposes this build capability to future auth UI. Production profile/identity is deliberately absent pending product and signing decisions. Root bootstrap also requires `EXPO_PUBLIC_API_URL` and `EXPO_PUBLIC_CLERK_PUBLISHABLE_KEY`; preview rejects non-HTTPS and loopback API origins. Current source has launch, auth bootstrap/recovery, Welcome, signup through verified authenticated completion, password Login, a shared authenticated shell with five tabs plus drawer, a production-backed Notifications tab with cursor pages/read-state controls, a production-backed Bookmarks drawer destination with folders/move/remove/search, a production-backed Lists drawer destination with public discovery and watchlist reads, a production-backed Profile tab plus profile routes with detail/tabs/connections/stats/share/follow/mute/block/report/media preview and standalone Edit Profile, a production-backed Chat drawer destination with inbox/thread/message/reaction/edit-delete/image-send/read/typing/presence flows, and a mixed-post feed/video-composer/comment-reader slice. Onboarding and full production implementations for Discover, list mutations/title watchlist actions, notification follow requests/realtime/deep links, native Chat Ably/GIF/file send/process restoration, Settings, Help/legal, Drafts, and 70mm remain gated. It contains no fake data or mock production screen and imports no `packages/ui`, Next.js, Radix, or DOM-dependent application source.
...
`packages/design-tokens` is the React-free Phase 1.5 source for semantic colors, all eight theme preferences, auto light/dark resolution, 4-point spacing, radii/sheet geometry, touch/avatar/icon/poster/media sizes, DM-family typography roles, motion/Reduce Motion behavior, and calibrated iOS/Android elevation recipes. Stable fixtures read and assert current mobile-web CSS and Swift `Theme.swift` palettes, including Letterboxd. Mobile reference hierarchy selects web behavior for recorded Matrix/Oppenheimer social-accent and Oppenheimer unread-badge conflicts while retaining both reference values in tests. Tests cover source parity, preference/auto resolution, four-point spacing, critical WCAG AA pairs, and Reduce Motion. Package has no React, React Native, DOM, UIKit, Android, API, DB, cache, queue, or network dependency; no production read/write path or index changes.
...
- `src/modules/catalog/readService.ts`: public catalog read/search service for title/person/company detail, indexed title/person slug lookup, compact search cards, credits, media, external IDs, aliases, relations, awards, company titles, edit queue, and public history. Reads use cursor pagination and DB-backed `sort_title` / `sort_name` prefix search.
...
- `/v1/catalog/titles/by-slug/:slug`
...
- `/v1/catalog/people/by-slug/:slug`
...
- Root layout owns pathname-aware scroll behavior: each pathname stores and restores its own bounded session scroll position, unvisited pages start at the top, and post-detail back navigation restores only the recorded source feed path. Shell layout adds auth bootstrap, onboarding gate, skip link, and the shared `ShellGrid`.
...
- Desktop home uses its reserved `xl` left rail for `ProfileCompletionWidget`. Its restrained four-detail progress card derives avatar/cover/bio/location completion from the existing cached `/v1/me` response, keeps completed details visible as subdued confirmation, hides the whole card at 100%, and deep-links each missing row to the owner profile edit dialog. Signed-out guests do not mount the rail. Validated `editProfile` query targets open, scroll to, focus, and briefly highlight the requested control; compact `CoverPhoto` editor mode keeps cover upload/crop inside the same dialog. `/v1/me` computes four booleans from its existing bounded profile row and adds no second shell query, index, cache, queue job, or schema change.
- Desktop home right rail renders `The Lobby` from `features/audio-rooms` above people suggestions. It is a UI-only, bounded audio-room summary contract, hidden for all viewers when no rooms exist; no production room API, signaling, media transport, or presence service is wired. The shared home/profile rail hides people suggestions after an empty response, including computing responses, while retaining the initial loading skeleton. Guests do not see the people-suggestions card. Visibility uses existing query data and adds no requests, writes, caches, jobs, schema changes, or indexes at 1M+ DAU. The rail caps output at four rooms; populated summaries show one host, at most three speaker avatars, a topic, and a denormalized listener count. No fake live data or inert join/create action ships.
...
Film logging was updated on 2026-09-13. `PostComposer` uses a single flow for title-page and searched-film entry: optional half-star rating, a local-calendar watched date, optional coarse watch venue, optional review of any length, and explicit feed-sharing. `StarRatingInput` is a labeled native range with half-step keyboard/pointer controls and a clear action, using the same green rating tokens as title-page stars. `useLogSharingPreference` defaults profile-only, isolates browser preferences by signed-in account, and remembers only successful saves. Edits preserve visibility and restore rating/date/venue/rewatch through every post-to-composer handoff.

`posts.watched_on` (nullable date), `posts.watch_venue` (nullable enum), and `posts.is_rewatch` (boolean) persist diary details; unknown legacy dates and venues remain null. `post_edits` records previous type, film, rating, date, venue, rewatch, and visibility alongside body/headline. Shared validators reject impossible Gregorian dates and invalid venue values, and web disallows future dates relative to the viewer's local day. Empty log/review text becomes a log; any visible text becomes a review. Metadata-only edits preserve legacy classification. Date fields and the partial unique creation-key plus watched-date diary indexes are in migration `0065_film_diary_watch_details`; watch venue is added in migration `0067_fuzzy_sunspot`.

`ProfileDiaryTimeline` requests `kind=diary`, so the API filters before pagination and orders by `coalesce(watched_on, UTC creation date)`, exact creation timestamp, then ID. Opaque diary cursors preserve microseconds and public diaries retain full pagination with existing access controls. UI date formatting never shifts date-only values by browser timezone. Profile stats share the effective watch date and add bounded venue distribution; cache namespaces advance to `feed-cache:v6` and `profile-stats:v5`.

Create retries reuse an immutable draft payload with `idempotencyKey`; `(user_id, creation_key)` uniqueness and request hashes prevent duplicate posts or changed-content reuse. Counter/fanout intents are transactional and created once. Safe post-commit effects rerun after an ambiguous response, with mention inbox rows deduplicated through the existing unique `notifications.source_key`. At 1M+ DAU this preserves bounded, authenticated/rate-limited writes, asynchronous counters, soft deletion, indexed `limit + 1` diary reads, and batched hydration; unchecking Share to feed leaves the entry public on the profile and skips follower-feed fanout without changing visibility.
...
- `InfinitePostList` uses React Query infinite pagination, velocity-aware prefetching, first-page-onward window virtualization, and memoized `PostCard`. Home/profile streams use `useFeed`; `/:username/post/:postId/quotes` uses its own post-and-sort-scoped query key while retaining the same 640px feed position. Keeping one render strategy avoids a structural layout swap when loaded history crosses a row threshold. Profile feed keys include `all | reposts | diary`; Reposts sends `kind=reposts` so the server filters before cursor pagination.
...
- `GET /v1/feed/profiles/:username/posts`: profile feed; optional `kind=reposts` returns only repost activity. Signed-out viewers receive a teaser only: `limit` capped at 5 (`PROFILE_FEED_GUEST_POST_LIMIT`), `nextCursor` always `null`, `cursor` param rejected with `401`; `hasMore` stays truthful. Web renders `ProfileGuestFeedGate` (skeletons + "See {name}'s full profile" signup banner) via `InfinitePostList`'s `guestFooter` on the Posts/Reposts tabs; Lists/Diary/Stats stay fully public.
...
- Native iOS activity uses `NotificationsViewModel` for cursor paging/read state, a reusable equal-width `HeaderTabBar` mounted inside the activity header for All/Unread filtering, and a native page-style `TabView` pager so All/Unread pages track the finger during horizontal swipes and stay synchronized with header taps. The standalone notifications destination reuses the same filter bar and pager above the list. Each filter owns a bounded cursor stream with idempotent initial loading, so normal swipes do not re-fetch; read-state mutations refresh the sibling filter after the user action so All/Unread stay consistent without unbounded reads. Full-width hairline borders separate rows; each row has an independent 44-point more-options control backed by the shared bottom sheet for existing post navigation and read/unread mutations. Actor, action, and relative time form one compact sentence, with secondary film/post context underneath and resolved media on the trailing edge. Kingfisher and existing bounded API payloads avoid per-row network or database fanout.
- The retained SwiftUI notification pager receives its All and Unread `NotificationsViewModel`s from `AppSessionViewModels`, so Activity tab revisits and standalone notification sidebar returns do not issue another initial page load unless the user explicitly refreshes, retries, mutates read state, or changes account.
...
- Appearance supports theme and accent color. Accepted theme values are `auto`, `light`, `dark`, `letterboxd`, `matinee`, `matrix`, `oppenheimer-bw`, and `barbie`.
...
- Public title pages live at `/film/[slug]` and `/tv/[slug]`; `/title/[media]/[id]` remains a permanent-redirect compatibility route. Person pages use name slugs and permanently redirect legacy numeric paths. Page content remains largely TMDB-oriented after catalog-first slug resolution.
...
- The iOS messages module has a native inbox and core thread experience backed by the same chat contract. Messages is not mounted in the bottom tab bar; `MainTabView` pushes it from the header message icon using each tab's `NavigationStack`, while the header avatar opens a stationary left profile sidebar populated from `/v1/me`. That response includes denormalized follower/following counts used beneath the username in both iOS and mobile web, with no extra sidebar query. `TraditionalTabBarEnabled` defaults on and renders an app-owned fixed full-width Home/Discover/Add/Activity/Profile bar so iOS 26 cannot reserve extra `UITabBar` layout space above the items; setting it false restores the previous system Home/Create/Activity shell. Add remains the composer trigger and returns to the previous durable tab. The traditional bar hides all visible item labels while keeping accessible labels; Home/Search/Compose/Activity use template-rendered bundled SVG assets and Profile renders the current user's avatar or initials fallback. Home feed down-scroll hides the app header and bottom tab chrome, while a small upward scroll or top lock restores both with the mobile-web timing curve. Discover, Activity, Bookmarks, and Profile apply the same upward-swipe/deeper-scroll hide gesture to header chrome only: downward swipe/back-scroll or top lock reveals it, and the bottom tab bar stays visible. Reduce Motion removes spatial chrome transitions. Opening the sidebar moves the full tab/header/bottom-bar page surface right by the drawer width without vertical movement, scaling, or clipping, using the mobile-web 300 ms timing curve, a dimmed close surface attached directly to the moving tab surface, a transparent native tab-bar backdrop in the system fallback path, and a no-animation Reduce Motion path. The module supports cursor-paged inbox reads, realtime `thread.updated` row patching, visible-thread typing subscriptions, batched visible-row presence, archived/default lists, native swipe actions, minimal profile-search DM creation, flipped `UICollectionView` thread rendering with message-id diffable identity, stable typing/loading/bottom-anchor items, `reconfigureItems` for unchanged message/read/typing state, top-edge older pagination, cancellable Kingfisher media prefetch, logical VoiceOver ordering, UIKit-targeted context menus, collection-coordinate reply jumps, realtime message/reaction/read/typing patching, read receipts, reaction toggles, optimistic send/retry, image/file attachment uploads through `/v1/media/presign`, sender-only edit/delete, throttled typing dispatch, and foreground-only read dispatch. Native GIF sending, jump-to-unloaded replies, per-member group read receipts, and richer group creation remain staged separately.
...
- Feed cache namespace: `feed-cache:v6`.
...
`apps/mobile/src/features/feed` now owns the mixed `FeedPost` renderer, native rich-text body, feed/detail/comment API adapters and query keys, comment tree builder, canonical post-route parser, and post-detail screen. `apps/mobile/src/app/post/[postId].tsx` accepts the canonical UUID used by `posts.id`; ULID-shaped identity remains specific to `films.id`. Non-control PostCard taps and the comment action open that route, while nested More/link/interaction controls remain independent. Multi-image PostCard media can opt into the same 2+-image peek carousel through `EXPO_PUBLIC_POST_MEDIA_CAROUSEL`; default and quote-preview media presentation remains the existing grid. Feed and comment requests are cursor-bounded to 20; FlashList virtualizes both surfaces; rich-text work is bounded by size, nodes, and depth; and loaded comments are deduplicated before linear three-level tree construction. The path reuses existing feed/post/comment APIs, denormalized counters, post soft-delete, and `comments_post_moderation_created_at_id_idx`; it adds no backend route, DB/Redis/cache/queue/worker work, migration, or index. Mobile comment creation remains unexposed until the existing rate-limited endpoint also satisfies the required server-side idempotency contract.

`apps/mobile/src/features/bookmarks` owns the React Native bookmark drawer destination. It parses `/v1/feed/bookmarks` and `/v1/feed/bookmarks/folders`, keeps All/Unsorted/folder filters plus loaded-page search in local UI state, stores cursor pages and folder summaries in React Query, and uses existing folder create/rename/delete plus bookmark move/remove endpoints. Move/remove optimistically patch bookmark cursor caches and roll back on error; the shared `PostCard` exposes an optional bookmark-action override so Home/feed behavior remains unchanged. Reads stay at 20 items per cursor page and reuse denormalized folder/unsorted counts. This adds no backend route, schema, migration, worker, cache, native dependency, or index.

`apps/mobile/src/features/lists` owns the React Native Lists drawer destination. It parses existing `FilmListPage`, `FilmListDetail`, and `FilmListEntry` envelopes, reads `/v1/lists` popular/recent cursor pages for Explore, and reads `/v1/lists/me/watchlist` for the signed-in Watchlist tab. State stays in React Query plus local tab/sort UI state; list create/edit/delete, entry mutations, reorder/notes, like/clone, and title watchlist actions remain gated. Reads reuse existing list indexes and pending-counter overlays; no backend route, schema, migration, worker, cache, native dependency, or index was added.

`apps/mobile/src/features/chat` owns the React Native Chat drawer destination. It parses existing `ChatInboxPage`, `ChatMessagesPage`, `ChatThreadPreview`, `ChatMessage`, typing, read-receipt, presence, contact-search, and media-presign contracts at the mobile trust boundary. State stays in React Query plus local composer/modal state. Inbox reads use `/v1/chat/inbox` cursor pages; thread history reads `/v1/chat/threads/:threadId/messages` with `before`; mutations reuse existing chat send/edit/delete/reaction/read/archive/mute/delete/typing endpoints; DM creation uses bounded `/v1/profiles/search`; image sends use `/v1/media/presign` and direct R2 PUT. Native Ably subscription, GIF/file sending, process restoration, visual/E2E, and low-end Android keyboard/list/media performance evidence remain gated. No backend route, schema, migration, worker, cache, native dependency, or index was added.
...
### 2026-09-21 — Retained SwiftUI app-session view-model cache

`AppEnvironment` owns `AppSessionViewModels`, an authenticated-user-scoped in-memory cache for retained SwiftUI feed, bookmarks, notifications, profiles, chat inbox, and chat thread view models. `MainTabView` injects these retained models into route destinations instead of constructing fresh loaders every time SwiftUI rebuilds a tab or navigation path. Feed/bookmarks use a 60-second freshness gate and chat inbox/thread surfaces use a 30-second gate: quick returns keep loaded rows/messages visible, while stale returns revalidate through the existing bounded cursor/read endpoints without full-screen skeleton resets. Notifications and profiles keep their already-loaded rows/detail/tab pages until explicit refresh, retry, read-state sibling sync, or account change. The cache clears when the active authenticated user ID changes, preventing cross-account data reuse. This is client-only state ownership; it adds no API route, schema/index, cache/worker behavior, mutation, pagination contract, or UGC lifecycle change.
