...
App-wide design tokens live in `apps/ios/ThirtyFiveMM/Core/DesignSystem.swift`: brand accent `#c2473a` (also wired into `AccentColor.colorset`), like/repost action colors, a 4pt spacing scale, corner-radius scale, canonical avatar sizes, and semantic Dynamic Type font aliases (`.appWordmark`, `.appScreenTitle`, `.appAuthorName`, etc.). Color themes mirror web `data-theme` values through `Core/Theme.swift` (`auto | light | dark | matinee | matrix | oppenheimer-bw | barbie`) with accent overrides; `ThemeManager` is local authority (UserDefaults, like web localStorage) — settings GET never overrides a persisted theme (that caused NEW→OLD→NEW flashes when Settings recreated mid-save). Appearance saves apply `ThemeManager` first, PATCH in the background, and merge the client request over stale responses. Switches snap `UIWindow.overrideUserInterfaceStyle` (not `.preferredColorScheme`) before publishing an atomic `ThemeSnapshot`, with static light/dark palettes. Window / tab-host / `themedBackground()` fill behind bottom chrome with the palette. The traditional flagged iOS path uses an app-owned safe-area bottom bar instead of system `UITabBar` items, and the fallback system tab path keeps `UITabBar` clear; SwiftUI must not use `.toolbarBackground(.visible, for: .tabBar)` on iOS 26 because it expands into a content-covering bottom slab. `SettingsViewModel` is app-scoped on `AppEnvironment` so it survives destination recreation; settings sections push as `AppRoute.settingsSection` on the tab `NavigationStack` path so theme rebuilds cannot pop Appearance. Active `ThemePalette` is injected via `@Environment(\.theme)` at `RootView`. Shell chrome, feed cards, notifications, and profile header/tab surfaces consume that palette (not `Color(.systemBackground)` / `.label`) so custom themes actually paint. Signed-out welcome may use its own hero art, but iOS auth form screens use neutral white/black/gray surfaces and standard system typography.
