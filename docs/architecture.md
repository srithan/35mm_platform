...
The native iOS launch screen uses `Resources/LaunchScreen.storyboard`, selected by `UILaunchStoryboardName` and included in target resources. Its system background and appearance-aware `LaunchWordmark` render white/black in light mode and black/white in dark mode before SwiftUI starts. This matches the adaptive in-app splash colors before account-theme restoration; changes require a rebuilt, signed native binary. Native launch screenshots were verified in both appearances on a separately identified simulator copy; physical-device behavior remains unverified. No backend traffic or schema change.
