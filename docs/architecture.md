...
Phase 1.2 adds Jest 29.7 through `jest-expo` 57.0.2 and React Native Testing Library 14.0.1. Tests live outside Expo Router's route directory, and Jest discovery is rooted at `src` so generated-native and local Expo cache artifacts cannot create unbounded enumeration work. `pnpm mobile:check` is the deterministic CI entry point and runs the design-token, API-client, and mobile-UI package checks, strict mobile TypeScript, Expo ESLint, public Expo config validation, quality-harness contracts, native policy/generation checks, and serialized Jest CI coverage. Current tests cover 29 source-backed token invariants, 6 API transport cases, and 78 app/UI cases spanning Router/config, variant capabilities, shared controls, provider composition/token injection, runtime rejection, state/lifecycle/retry boundaries, persisted-cache policy/account cleanup, launch and auth-bootstrap recovery, Welcome content/navigation/legal behavior, signup identity/email/password/DOB validation, draft migration/privacy/restoration, volatile-secret process recreation, username debounce/cancellation/error recovery, Clerk payload privacy/resume, authenticated DOB completion sequencing, password visibility/autofill metadata, duplicate-action blocking, and the internal foundation accessibility/state gallery.
...
10. Continue React Native Phase 2 with Login and Clerk session-task/MFA handling. Email verification, resend cooldown, change-email recovery, Clerk session activation, and the authenticated DOB completion bridge are implemented. Embedded-bundle iPhone build/sign/install, visible gallery rendering, sustained process, and clean startup logs are verified; representative physical Android evidence remains deferred by user and must still precede public release.
...
See [Bunny video integration](./bunny-video-uploads.md) for route contracts, quota and retention details, account configuration, operational limits, and the 1M-DAU capacity assumptions. Native/mobile video clients and unrelated existing 70mm mock shelves are not changed by this web/API integration.
