...
and preview build identities, enforceable Expo CNG policy, and React-free
shared design tokens plus React Native UI primitives, root providers, bounded
account-scoped persistence, Clerk secure token wiring, lifecycle handling, and
a platform-neutral API client plus accessibility, Maestro E2E, PNG visual-diff,
and measured-performance evidence harnesses; `apps/ios` remains intact as the SwiftUI
reference and retained fallback. The local CocoaPods/Android/Maestro toolchain is
configured; the Android development binary plus emulator smoke flow pass, and a
Personal-Team-signed iOS embedded-bundle build now builds, installs, launches,
and visibly renders the Foundation gallery on the connected iPhone 13 Pro
without Metro. Device syslog first proved its black screen came from Release
dead-code stripping the generated `ExpoModulesProvider`, leaving `ExpoAsset`
and Expo Constants absent. An explicit CNG plugin lifetime-retains that
provider from generated `AppDelegate`. A second startup failure exposed Expo's
unquoted `PROJECT_DIR` handling under repository paths containing spaces,
which silently omitted the embedded Expo Constants manifest. The retained
Podfile plugin plus pinned dependency patch now supply and safely preserve the
application root. The final app contains its JS bundle, valid Expo config, and
provider class; strict signing, visible screenshot, sustained process, and
clean device-log checks pass. Native verification now executes Expo manifest
generation for both variants through a synthetic path containing spaces, while
a macOS Release-artifact gate rejects missing JavaScript/config, mismatched
identity, stripped provider symbols, or invalid signing before installation.
The root bootstrap also no longer returns blank
trees during Clerk, query-scope, font, or theme initialization: it renders one
explicit loading surface and does not block routes on non-critical theme
hydration.
Expo SDK 57's generated Debug app still reaches React Native with a null bundle
URL before launcher UI appears and is not used for physical-iOS smoke validation.
Phase 1.9 remains blocked on one low/mid-range physical Android device.
...
│   ├── api-client/  Platform-neutral mobile REST transport and errors
│   ├── design-tokens/ React-free shared mobile themes and foundation tokens
│   ├── mobile-ui/    Shared React Native primitives and state surfaces
...
The tree above reflects implemented workspaces. `apps/mobile` currently contains
Phase 1.1 runtime/configuration and root Router entry infrastructure, Phase 1.2
test/CI command wiring, Phase 1.3 build-variant validation, and Phase 1.4 native
generation policy/verification. Phase 1.5 adds the shared design-token package
and source-backed web/Swift theme parity fixtures. Phase 1.6 adds the shared
React Native UI package, local DM font assets, one cross-platform icon source,
overlays, toast, skeletons, and explicit state surfaces. Phase 1.7 adds root
provider/bootstrap composition, Clerk secure token caching, React Query and
Zustand ownership boundaries, bounded account-scoped persistence, lifecycle
...
- `apps/mobile`: Expo development/native builds with isolated internal EAS development and preview profiles; production identity, signing, and store profile remain blocked on release decisions.
...
pnpm mobile:test
pnpm mobile:test:ci
pnpm mobile:tokens:check
pnpm mobile:api-client:check
pnpm mobile:ui:check
pnpm mobile:quality:check
pnpm mobile:e2e:foundation
pnpm mobile:visual:test
pnpm mobile:performance:verify
...
Web development uses `next dev` (webpack in Next.js 15), matching the production build. The existing webpack `resolve.extensionAlias` maps Node-compatible `.js`/`.mjs` imports in shared TypeScript packages to their source files. Do not add `--turbopack` without equivalent extension resolution; it bypasses this configuration and breaks imports such as `@35mm/validators`' `./dateOfBirth.js`.
...
- PostComposer footer groups Bold, Italic, Underline, Strikethrough, and Spoiler behind one formatting icon. Hover opens a horizontal portaled menu above the trigger; click/touch and keyboard access remain available. The menu avoids footer overflow clipping, sits above the composer modal using `--z-composer + 2`, and uses existing TipTap commands. This local UI change adds no API reads/writes, cache, jobs, schema, or indexes at 1M+ DAU.
...
- `NEXT_PUBLIC_MOBILE_TAB_BAR_TRADITIONAL` gates the mobile bottom tab bar
  ...
  cache, worker job, schema, or index.
- `TraditionalTabBarEnabled` gates the retained SwiftUI iOS bottom tab bar
  through Info.plist, `TRADITIONAL_TAB_BAR_ENABLED`, or UserDefaults key
  `traditionalTabBarEnabled`. Default `true` uses an app-owned fixed
  full-width native tab bar with icon-only Home, Discover, Add, Activity, and
  Profile items; Add opens the composer and returns to the previous durable tab.
  Reselecting an active durable tab clears only that tab's typed
  `NavigationStack` path, so Home returns from Bookmarks or another pushed
  destination to Feed while inactive tabs retain independent history.
  Every item keeps its accessible label, Home uses the provided home asset,
  Discover uses the provided search asset, Add uses the provided compose asset,
  Activity uses the provided heart/activity asset, and Profile renders the
  current user's avatar or initials fallback instead of a generic person icon.
  These tab assets are template-rendered from the bundled SVG source so active
  and inactive colors still come from the current theme. The flagged
  path avoids system `UITabBar` layout reservation so iOS 26 cannot insert the
  tall content-covering slab above tab items. Home feed down-scroll hides both
  the header and bottom chrome; any small upward scroll or return to top restores
  them, matching mobile web while honoring Reduce Motion. Discover, Activity,
  Bookmarks, and Profile apply the same upward-swipe/deeper-scroll hide gesture
  to header chrome only: downward swipe/back-scroll or top lock reveals it, and
  the bottom tab bar stays fixed. Set it to `false` to restore the previous system
  Home/Create/Activity chrome. This local chrome flag adds no API read/write,
  cache, worker job, schema, or index.
- `NEXT_PUBLIC_POST_MEDIA_CAROUSEL` gates the web PostCard image presentation
  ...
  images and the existing carousel for five or more images. `true` renders
  multi-image PostCard photos as a horizontally swipeable peek carousel with a
  left-aligned position indicator above the photos. PostComposer quoted-post
  previews continue to use the existing quote grid. The same presentation is
  opt-in on React Native through `EXPO_PUBLIC_POST_MEDIA_CAROUSEL` and on the
  retained SwiftUI app through `PostMediaCarouselEnabled` / environment
  `POST_MEDIA_CAROUSEL_ENABLED` / UserDefaults `postMediaCarouselEnabled`.
  Retained SwiftUI defaults this flag on in `Info.plist`, applies the carousel
  only when a post has more than two images, and can revert to the grid with
  `POST_MEDIA_CAROUSEL_ENABLED=0` or UserDefaults `postMediaCarouselEnabled=false`.
  These are client-only presentation flags over already-loaded post media and
  add no API read/write, cache, worker job, schema, or index.
- `NEXT_PUBLIC_FOCUSED_NAVIGATION` is an opt-in, compile-time web UI flag. When
  `true`, desktop replaces `SiteHeader` with one fixed left navigation rail
  containing brand, Home, Discover, Watch, Messages, Activity, Profile, and
  the existing post composer trigger. It suppresses the duplicate Browse rail
  menu while retaining the profile-setup widget. Home drops its empty left
  desktop grid track, leaving content and right-space tracks beside the fixed
  navigation rail. Discover, Films, Lists, movie-title, and people routes use
  one centered content column without auxiliary rails, capped at 1240px.
  Post-detail's 640px column compensates for half rail width so it remains
  centered in full desktop viewport.
  Discover, Films, and Lists additionally share a desktop-only, three-option
  top navigation strip; mobile remains unchanged.
  The bottom `More` control reuses every existing profile-dropdown action and
  settings subview. Mobile navigation is unchanged; default `false` preserves
  the current desktop header. This is
  presentation-only and adds no API read/write, cache, worker job, schema, or
  index. At 1M+ DAU it adds only bounded client-side markup and event handling,
  with zero incremental backend read/write volume.
- `NEXT_PUBLIC_FOCUSED_NAVIGATION_CENTERED` is an independent, opt-in,
  compile-time presentation flag for the focused desktop rail. When `true`, it
  vertically centers the primary navigation stack while the brand stays at the
  top and the secondary account actions stay at the bottom. Default `false`
  preserves top-aligned primary navigation. It adds no API read/write, cache,
  worker job, schema, or index.
...
App-wide design tokens live in `apps/ios/ThirtyFiveMM/Core/DesignSystem.swift`: brand accent `#c2473a` (also wired into `AccentColor.colorset`), like/repost action colors, a 4pt spacing scale, corner-radius scale, canonical avatar sizes, and semantic Dynamic Type font aliases (`.appWordmark`, `.appScreenTitle`, `.appAuthorName`, etc.). Color themes mirror web `data-theme` values through `Core/Theme.swift` (`auto | light | dark | letterboxd | matinee | matrix | oppenheimer-bw | barbie`) with accent overrides; `ThemeManager` is local authority (UserDefaults, like web localStorage) — settings GET never overrides a persisted theme (that caused NEW→OLD→NEW flashes when Settings recreated mid-save). Appearance saves apply `ThemeManager` first, PATCH in the background, and merge the client request over stale responses. Switches snap `UIWindow.overrideUserInterfaceStyle` (not `.preferredColorScheme`) before publishing an atomic `ThemeSnapshot`, with static light/dark palettes. Window / tab-host / `themedBackground()` fill behind bottom chrome with the palette without force-loading hidden UIKit controllers during switches. The traditional flagged iOS path uses an app-owned safe-area bottom bar instead of system `UITabBar` items, and the fallback system tab path keeps `UITabBar` clear; SwiftUI must not use `.toolbarBackground(.visible, for: .tabBar)` on iOS 26 because it expands into a content-covering bottom slab. `SettingsViewModel` is app-scoped on `AppEnvironment` so it survives destination recreation; settings sections push as `AppRoute.settingsSection` on the tab `NavigationStack` path so theme rebuilds cannot pop Appearance. Active `ThemePalette` is injected via `@Environment(\.theme)` at `RootView`. Shell chrome, feed cards, notifications, bottom action sheets, and profile header/tab surfaces consume that palette (not `Color(.systemBackground)` / `.background` / `.label`) so custom themes actually paint. Signed-out welcome may use its own hero art, but iOS auth form screens use neutral white/black/gray surfaces and standard system typography.
...
Native feed post cards mirror the shared REST feed contract through `Core/Models/FeedPost.swift`, `PostInteracting`, and `Features/Feed/PostCard.swift`. Film-log cards consume canonical film ID, poster, genres, and normalized 0.5-5 rating metadata already present in that payload; poster-derived card color is computed on-device from a 16x16 sample and kept in a bounded memory cache, adding no feed API or database read. Likes, reposts, bookmarks, comments navigation, and poll votes use the same `/v1/feed/posts/:postId/*` endpoints as web. Poll voting is optimistic on-device and relies on the API's idempotent vote fact row plus async `counter.increment` jobs for durable poll totals/options, so the hot request path does not perform synchronous counter updates. PostCard media with more than two images can opt into `PostMediaCarousel.swift`, a horizontally scrolling peek carousel with a compact left-aligned dotter above the photos; one- and two-image posts keep the existing `PostMediaGrid`, and quoted-post previews continue to use the grid. Native post detail renders the complete post body, keeps TipTap paragraph breaks at one rendered line break, places the reply trigger/expanded composer inline above the cursor-paged comment list, and gives comments a compact like/reply-count/reply action row backed by the existing optimistic comment-like and create-comment paths. Native URL previews share web's image-first editorial card and text-only fallback; YouTube/Vimeo post links and YouTube/Vimeo/Dailymotion comment links use locally detected, de-duplicated video thumbnail cards. Detection is bounded to each already-loaded body and publisher thumbnails load through the existing Kingfisher image path, adding no API, database, cache, queue, worker, schema, or index. Post and comment identity links share the same compact role/context/films-logged headline directly beneath the name. Their shared native identity/timestamp header uses Twitter-parity Dynamic Type tokens from `Core/DesignSystem.swift`: bold subheadline (15pt) name, regular subheadline handle and inline muted `· timestamp` at the same size, optional footnote `logged` context, and trailing overflow control. Feed bodies render at 15pt (`.appBody`) with 3pt line spacing; detail bodies render at 17pt (`.appBodyLarge`). Feed timestamps use the same `now`, minute, hour, and unbounded day buckets as web; edits do not prepend a competing header label. The native `ShareModal` matches mobile web's branded 32pt-radius bottom sheet for post and profile sharing, including the content preview, ten-destination grid, separator, copy-link feedback, synchronized backdrop/panel animation, drag dismissal, safe-area padding, and Reduce Motion behavior. Its regular-height shell has no outer height constraint and therefore resolves strictly to its content plus safe-area padding; only the inner compact-height scroll region receives a cap for phone landscape. The complete sheet remains composed offscreen before presentation and moves as one container; its fixed ten-item grid uses eager rows so preview, destinations, and copy control cannot enter on separate layout passes. Share destinations and previews are composed entirely on-device from already-loaded data, adding no API, database, cache, queue, or media request beyond optional preview-image delivery. Existing comment list/create/edit responses expose those fields from their already-joined profile row, adding no query, N+1 read, route, cache, job, schema, or index. Author avatar and complete display-name/username/role clusters are explicit buttons that push the shared `ProfileView` through the shell's typed `AppRoute` path; they remain independent from full-card post navigation. An explicit profile tap adds one bounded existing profile read and no feed/comment query. By default, the retained native bottom tab bar exposes Home, Create, and Activity; Messages/Profile are not bottom-tab destinations. `TraditionalTabBarEnabled` keeps that shell recoverable while switching bottom chrome to a fixed full-width Home, Discover, Add, Activity, and Profile tab set. Add opens the composer and restores the previous durable tab; in the traditional bar every visible text label is suppressed while accessible labels remain, Home/Discover/Add/Activity use selected filled plus inactive outline SVG variants at the same 24pt template glyph size, and Profile renders the current user's avatar or initials fallback. Discover and Profile reuse their existing bounded feature surfaces. Home feed scroll chrome follows mobile web: a shared `UIScrollView` pan observer reports finger translation and velocity to the shell, hiding header and tab bar on down-scroll and restoring both near the top or on slight upward movement. Discover, Activity, Bookmarks, and Profile reuse the observer with header-only behavior: upward swipe/deeper scroll hides their page/header chrome, downward swipe/back-scroll or top lock reveals it, and the bottom tab bar remains visible. The Home header and traditional tab bar slide/fade in independently animated overlay layers while the Home scrolling surface extends through the bottom safe area. Both feed insets stay fixed across traditional-bar visibility changes, including bottom clearance for the bar and safe area; this preserves last-row reachability and prevents UIKit from shifting the feed when its scroll range changes at the tail. The traditional Home visibility callback does not animate the feed subtree; only the header and tab overlays receive the chrome animation. Reduce Motion disables those animations. The app shell header loads the current `/v1/me` profile once, including denormalized follower/following counts, opens a stationary left profile sidebar from the avatar, and pushes the complete tab/header/bottom-bar surface right by the drawer width with the same horizontal-only 300 ms timing curve and dimming as mobile web. Both native and mobile-web sidebars show those relationship counts beneath the username without another query. The native tab bar uses the system default (blurred) backdrop with label/secondary-label item tints and selection-aware filled/outline SF Symbols unless the traditional flag is enabled, in which case it uses an opaque full-width themed background and border. The sidebar push does not apply clipping or vertical layout modifiers. Reduce Motion disables that spatial animation. Messages remains a `NavigationStack` destination opened from the header message icon. `AppEnvironment` also owns account-scoped retained SwiftUI session view models for feed, bookmarks, chat inbox, and chat threads; the cache resets on authenticated-user changes, and each surface uses short freshness gates so returning within seconds keeps loaded rows visible while stale data revalidates through the same bounded cursor/read endpoints.

Retained SwiftUI Home and Profile Posts/Reposts now render loaded feed rows through `Features/Feed/FeedCollectionView.swift`, a `UICollectionView` bridge with compositional self-sizing rows and a diffable data source keyed only by stable post ID. Profile keeps `ProfileTabPager` as the outer horizontal paging owner; the shared collection renderer runs in non-scrolling embedded mode for those tabs and reports measured content height back to the pager. Content changes compare a render fingerprint and apply `snapshot.reconfigureItems` so optimistic like/repost/bookmark/poll/counter changes update cells in place instead of remove/insert churn. Shape-specific reuse identifiers (`text`, `review`, `media`, `poll`) currently host the existing SwiftUI `PostCard` via `UIHostingConfiguration`; the registration boundary is intentionally ready for later UIKit bodies for hot text/media cards. The controller loads its view before the first configuration, applies nonanimated serialized snapshots only when content/identity/presentation changes, and reconfigures changed surviving IDs even when a page also inserts rows (shape changes reload the matching reuse type). Hosted rows use required-width, natural-height fitting with post-scoped SwiftUI identity; UIKit owns size measurements, replacing the unused post-height cache. Hosted cells expose zero safe-area insets: viewport clearance stays on the collection, preventing row heights and spacing from changing as cells cross screen edges during scrolling. An idle reading anchor survives subsequent self-sizing passes and releases when dragging or scrolling to top. Overlay insets have one owner, initial content starts below the Home header, and chrome direction uses inset-normalized offsets plus active pan input. Embedded Profile height callbacks are deferred and deduplicated. `UICollectionViewDataSourcePrefetching` starts/cancels Kingfisher prefetch around upcoming rows while triggering cursor pagination five rows before the committed tail; refresh/snapshot application suppresses premature requests and a settled visible-tail check resumes eligible pagination without repeat requests for the same tail. Kingfisher has explicit memory/disk caps and feed images use display-target downsampling. At 10M DAU this renderer fix adds zero backend reads/writes or index requirements: feed reads remain existing 20-post cursor pages over hybrid fan-out and server caches, and mutations retain authorization, rate limits, soft deletion, and asynchronous counters.

Native Home initial loading also handles the retained-to-cold-history cursor boundary: the API may legitimately return a single retained post with `hasMore=true`, then older posts on the next cursor. `FeedViewModel.loadInitial()` stages fewer than six unique posts while fetching at most one continuation at the existing limit of 20, then publishes one initial array. The skeleton remains until that bounded handoff completes; full pages and exhausted one-post feeds publish immediately. A continuation failure exposes the successful first page plus an error and retains its retry cursor; cancellation publishes no staged rows. Repeated/missing continuation cursors stop pagination with an explicit error. Privacy-safe `FeedLoading` OSLog events report counts/stages, never tokens, cursors, bodies, or user IDs. This moves an already-immediate pagination request ahead of first rendering for short nonempty pages and also allows one continuation for an empty filtered page. It introduces no server route, query shape, cache change, schema, or index; cold client startup is bounded to two 20-item reads, ordinary startup remains one, and no unbounded fill loop exists.

Post images opened from Home, Profile Posts/Reposts, or Bookmarks use `Features/Post/PostImageViewerPresenter.swift` instead of a system full-screen cover. Each actionable media tile installs a non-interactive UIKit anchor inside its hosted SwiftUI body, captures the exact window-space frame plus the already-processed Kingfisher memory-cache image at tap time, and presents the existing SwiftUI viewer through a custom `UIHostingController` transition. Presentation springs one snapshot from the tile to the viewer image's measured UIKit anchor. The pager owns the full-screen viewport, avoiding separate page safe-area offsets; the live image stays covered until the snapshot lands. Shorter full-width images are vertically centered between the reserved top and bottom controls through the shared image-frame calculation; oversized images retain their existing top inset and full width. A downward drag translates and scales only a cached image snapshot in stationary container coordinates. The live image is covered while the stationary viewer chrome and full-screen backdrop fade quickly, revealing the feed without dragging controls or black image padding. On release, dismissal starts at the actual transformed image frame and returns to a still-visible source anchor; a short, cancelled, or upward-reversed drag restores the image snapshot and viewer opacity before removing the snapshot and live-image cover. Directional failure priority is negotiated with nested scroll recognizers as they appear, including lazily created carousel pages. Zoomed images and presentations already in progress reject downward dismissal. A page without its original live source uses its cached current image and a centered downward offscreen target instead of crossfading the complete carousel. Viewer chrome uses compact 38pt visual circles inside 44pt hit targets, toggles as one layer when the image/backdrop is tapped, and each page supports bounded 1x–4x pinch zoom. Paging and downward interactive dismissal pause while zoomed. Missing cached imagery and Reduce Motion use the opacity fallback. Comment and quote routes are queued until dismissal finishes. This is bounded per-device presentation state over already-loaded media: it creates no backend read/write, API route, cache, queue, worker job, schema, or index at 1M+ DAU.

Retained SwiftUI chat threads use a flipped `UICollectionView` bridge for the message surface instead of `ScrollViewReader`/`ScrollView`. Diffable item identity is message-id based, typing/loading/bottom-anchor rows are stable data-source items rather than supplementary headers or footers, and unchanged message/read/typing state updates flow through `reconfigureItems`. Offset zero is the bottom/newest edge; older-message pagination and image/GIF/link preview prefetch run through `UICollectionViewDataSourcePrefetching`, with prefetch cancellation when rows scroll past. The bridge remaps visible VoiceOver traversal to logical oldest-to-newest order, anchors UIKit context menus against unflipped cell content previews, keeps keyboard avoidance outside the collection view through the SwiftUI composer safe-area inset, and resolves replied-to jumps through diffable message IDs in collection coordinates. The sharp-edge note lives in `docs/ios-chat-thread-rendering.md`. This is client rendering over existing bounded chat REST/realtime contracts and adds no API route, schema, cache, queue, worker job, or database index.

Retained SwiftUI pushed profile pages opened from post/profile identity links suppress the regular app header, extend the profile cover/loading skeleton behind the status bar, and use a translucent 44pt over-cover Back control while preserving the `NavigationStack` edge-swipe back gesture. The loaded screen captures the status-bar inset before extending under it and reserves a 50pt compact header in the scroll safe area. A single native pinned section supplies the tabs (no duplicate overlay tabs). As the viewer scrolls, the cover image blurs into that compact top header, profile name/count text fades into that header, and the Posts/Reposts/Diary/Stats/Lists tab controls pin directly underneath the compact cover header. The root Profile tab keeps its app header and scroll-hide behavior. This is local chrome behavior over the existing bounded profile read and adds no API route, database query, cache entry, worker job, schema, or index. Retained SwiftUI profile collapse progress observes the outer native UIScrollView contentOffset and adjustedContentInset through the existing refresh-control bridge. Updates are coalesced on the main queue and observers detach with the view; SwiftUI geometry preferences no longer control header opacity. The blurred compact cover has an opaque backing; header and tab geometry remain unchanged. Profile name and share/more source frames are measured in stable content coordinates; each compact counterpart reveals as its source crosses the header edge. Cover blur and fades use scroll-driven smoothstep easing; Reduce Motion suppresses added vertical reveals.

Retained SwiftUI feed revalidation uses `Features/Feed/FeedServicing.swift` and `FeedViewModel.swift` to merge the fresh first cursor page over already loaded rows instead of replacing the whole array during tab returns, post-detail returns, or idle freshness checks. Initial empty-state loads still replace from the first cursor page. This is client-side reconciliation over the existing bounded `/v1/feed` read and adds no API route, database query, cache entry, worker job, schema, or index.
...
Profile Posts/Reposts pass the visible profile's stable user ID and normalized username through `FeedCollectionView` into each hosted `PostCard`. Author avatar and full identity controls for that same user remain readable but inert, so repeated taps cannot stack duplicate copies of the profile already on screen. The shell also ignores an exact route already at the top of a tab path, providing a second guard against repeated profile pushes. Cards authored by another user and cards outside profile context retain typed `AppRoute.profile` navigation. This is local route gating over already-loaded identity data and adds no backend read, write, cache, job, schema, or index.

Retained SwiftUI app-session caching now covers notifications and profile pages in addition to feed, bookmarks, and chat. `MainTabView` asks `AppSessionViewModels` for the current account's notification pager models and username-keyed `ProfileViewModel`s, then injects those models into tab, sidebar, and pushed destinations. Returning to Activity/Profile therefore keeps already-loaded rows and profile tab pages in memory; explicit pull-to-refresh, Retry, read-state sibling sync, and account changes still perform intentional bounded reads.
...
Phase 1.1 implements private workspace `@35mm/mobile` with Expo SDK 57.0.8, React Native 0.86.0, React 19.2.3, Expo Router 57.0.8 typed routes, strict TypeScript, Expo flat-config ESLint, and automatic pnpm monorepo resolution. React type packages are excluded from pnpm's hidden workspace hoist, and existing React Email packages declare their missing React 18 type peers, preventing Expo's React 19 types from crossing into React 18 workspaces. Expo SDK 57 runs only on New Architecture/Fabric; generated Android config also records `newArchEnabled=true` and Hermes. Native build properties enforce Android minimum API 24 and compile/target API 36. The initial Expo iOS 16.4 floor was raised to iOS 17.0 in Phase 1.7 for the selected Clerk Expo native SDK.

Phase 1.3 requires explicit `APP_VARIANT=development|preview` during Expo config resolution and fails closed when the value is missing or unsupported. Development resolves to `35mm Dev`, `thirtyfivemm-dev`, and `com.thirtyfivemm.mobile.dev`; preview resolves to `35mm Preview`, `thirtyfivemm-preview`, and `com.thirtyfivemm.mobile.preview`. Both native identities remain distinct from SwiftUI production ID `com.35mm.app`. Cross-platform local scripts inject the development variant without shell-specific assignment. `apps/mobile/eas.json` maps internal development and preview profiles to matching EAS environments and variant values; no production profile exists while product/signing ownership remains blocked. Expo Dev Client is enabled for development builds, while its generated scheme is disabled for preview so installed variants do not compete for the same development URL. Development launcher mode disables automatic recent-bundle launch, but physical-iOS evidence shows that setting alone does not stop Expo SDK 57's generated Debug app from supplying React Native a null bundle URL before launcher UI appears. Clerk's supported `appleSignIn` plugin option is also variant-specific: development omits the entitlement for Apple Personal Team provisioning, preview retains it for paid-team validation, and public config exposes `extra.appleSignInEnabled` so future authentication UI can match the installed native capability.

`pnpm --filter @35mm/mobile config:check` resolves both public configs, checks exact display names, schemes, iOS/Android identifiers, public variant markers, profile-to-environment mapping, and cross-variant uniqueness. Unit tests also cover both identities and missing/invalid variant rejection. No product route, provider, auth flow, data client, feature implementation, API request, database load, cache, worker job, schema, or index was added; backend read/write volume and 1M+ DAU characteristics are unchanged. `packages/ui` and other DOM-only source are not imported.

Phase 1.4 adopts Expo Continuous Native Generation as source of truth. `apps/mobile/ios` and `apps/mobile/android` remain ignored, untracked, and disposable; retained native behavior must live in reviewed app config, explicit config plugins, or native modules outside generated trees. `apps/mobile/NATIVE_GENERATION.md` records adoption/review rules and local clean-regeneration commands. Policy validation rejects tracked generated files and symbolic-link targets, requires the independently tracked SwiftUI project to retain approved production identity `com.35mm.app`, and baselines explicit plugins plus resolved autolinking. Repository paths containing spaces are supported by a pinned `expo-constants@57.0.17` dependency patch that quotes both its CocoaPods script path and `PROJECT_DIR`, a Podfile plugin that supplies the application `PROJECT_ROOT` before podspec evaluation, and a structured config plugin that quotes the generated React Native bundle-script invocation. A pinned `expo-modules-jsi@57.0.8` patch disables signing only for its nested XCFramework build so File Provider metadata cannot invalidate that intermediate dependency; final app/framework signing remains intact. Another reviewed plugin injects a lifetime-held generated `ExpoModulesProvider` instance into `AppDelegate`, preventing Release dead-code stripping from removing Expo Asset/Constants module registration. Native checks execute the installed Expo manifest generator for both variants through a synthetic path containing spaces, then isolated clean Prebuild verification generates development and preview projects under OS scratch storage and checks their identities, display names, New Architecture, Hermes, iOS target, opposite Apple Sign-In entitlement states, recent-bundle auto-launch preference, Expo Constants project root, retained Expo provider, and safe bundle invocation. A separate macOS artifact gate validates embedded JavaScript/config, final identity, provider linkage, and strict signing. Scratch projects are removed without writing either workspace native tree. Native dependency/config changes must update this baseline and create new binaries rather than JavaScript-only OTA updates.

Phase 1.5 adds private package `@35mm/design-tokens` as plain TypeScript with no React, React Native, DOM, UIKit, or Android dependency. It owns eight preference IDs (`auto | light | dark | letterboxd | matinee | matrix | oppenheimer-bw | barbie`), seven resolved palettes, explicit system light/dark resolution for `auto`, semantic colors, 4-point spacing, radii and sheet geometry, touch/avatar/icon/poster/media sizes, DM-family typography roles, motion plus Reduce Motion substitutions, and equivalent iOS/Android elevation recipes. Stable fixtures read current web CSS and Swift `Theme.swift` during tests. Mobile-web values win the recorded Matrix/Oppenheimer social-accent and Oppenheimer unread-badge conflicts under the mobile reference hierarchy; both source values remain asserted so drift cannot be silent. Critical text/control pairs pass WCAG AA contrast, including readable theme-specific badge foregrounds. This static client data adds no API, database, cache, queue, or runtime network work, changes no 1M+ DAU read/write path, and requires no index.

Phase 2 configures `expo-splash-screen` through CNG for both internal variants. iOS and Android use static device-adaptive launch surfaces: white with a black local 35mm wordmark in light mode, black with its white variant in dark mode, rasterized from the retained SwiftUI launch assets; no remote media, API data, personalization, or JavaScript animation enters the OS-owned surface. The root holds the native splash until its first React layout, hides it once, and presents the same 151-by-56-point wordmark and background during Clerk, query-cache, and font bootstrap. There is no artificial delay or motion. This native dependency/config change updates exact plugin/autolinking baselines and requires regenerated binaries, while adding no server read/write volume or database index.

The Phase 2 root auth gate treats Clerk restoration, local API bootstrap, and onboarding status as separate states. It withholds signed-out rendering until Clerk is loaded, then performs cancellable parallel protected reads of `/v1/me` and `/v1/me/onboarding-status`, runtime-validates both trust boundaries, and resolves signed-out, onboarding, or authenticated state. React Query owns the memory-only result under an account-keyed factory. Offline/timeout and other failures remain on an explicit recovery surface with duplicate-safe retry and Clerk sign-out; sign-out cancels/removes bootstrap cache and resets transient UI state. Both reads use the existing unique `profiles.user_id` lookup, so signed-in starts add two bounded indexed reads, no N+1 work, mutation, new server cache, worker path, or index.

The signed-out Phase 2 destination is a device-adaptive Welcome screen matching the retained SwiftUI Intro reference. Nine bundled posters repeat in three bounded, independently animated Reanimated columns with top/bottom fades; motion stops in background and under Reduce Motion. The bare wordmark, italic cinema heading, and black/gray account actions use system typography. Standard layouts are fixed, with scrolling for enlarged text and compact height; startup performs no remote-media or API read before an account action. The primary and secondary actions route to the production signup-name and login paths, while Terms and Privacy can open only fixed 35mm HTTPS URLs and report failures accessibly. The deterministic foundation gallery also lives at internal route `/quality/foundation`; development and preview Maestro flows open that route explicitly instead of depending on the root destination.

Phase 2 signup owns `/signup/name`, `/signup/username`, `/signup/email`, `/signup/password`, `/signup/dob`, and `/signup/verify` under `features/auth/signup`. All six routes use one device-adaptive `SignupStepScaffold` for safe-area/keyboard handling, accessible six-step progress, typography, large-text wrapping, and back navigation where the state can safely move backward. Signup content is top-aligned and deliberately omits the prior cinematic hero on form steps to match the Spotify/Pinterest/Instagram prompt-first references. Name collects only full/display name; Username collects only lowercase username, restores bounded non-secret fields from a versioned AsyncStorage draft, and never persists password, token, Clerk resource, or availability state. Mobile imports the canonical username contract through the focused runtime-safe `@35mm/validators/username` export. React Query debounces public `/v1/usernames/:username/available` reads by 450 milliseconds, passes cancellation through the injected API client, drops inactive typed-name queries immediately, rejects malformed responses, blocks continuation until the current username is confirmed available, and exposes connection/server failure as an explicit retry rather than success.

The Email screen extends that draft to schema version 2 with a backward-compatible migration for identity-only drafts. Focused runtime-safe export `@35mm/validators/email` trims, lowercases, bounds to 254 characters, and validates the address accepted later as Clerk's `emailAddress`; Clerk remains authoritative when the account attempt is created. The shared React Native field sets email keyboard, autofill, return-key, autocorrect, and iOS content-type metadata, announces errors, preserves back/process-recreation state, and blocks duplicate continuation before routing to Password.

The Password screen adds production route `/signup/password` on the same scaffold. Shared `PasswordField` presentation supplies accessible show/hide state, secure entry, password-manager/autofill metadata, iOS password rules, and a 44-point visibility action. Feature validation preserves password bytes exactly, requires at least 8 characters, announces errors, and blocks duplicate continuation. Password memory lives only in the volatile portion of the existing Zustand signup store: back navigation retains it, AsyncStorage partialization excludes it, and process recreation requires re-entry. The current Clerk native environment was checked directly before implementation and enables password plus email-code signup with breach/strength checks; no environment response or secret is retained. Clerk remains authoritative at the later account-creation action, and this step routes to the separately owned DOB flow without creating an account.

The DOB screen presents a formatted date control and a shared bottom wheel sheet with Done, locale ordering, bounded virtualized years from 1900, leap-day clamping, and future-date limits. Confirmed dates persist in the non-secret draft before navigation. It normalizes only a valid calendar value to `YYYY-MM-DD`, and rejects impossible or future dates without timezone conversion. Signup draft schema 4 persists bounded non-secret DOB, email-code send time, and verified-completion Clerk user ID while passwords and verification codes remain memory-only. The final DOB action creates or safely resumes the exact matching Clerk signup attempt, sends no DOB through Clerk metadata, clears password memory after account creation, prepares email-code verification, and retains DOB for explicit retry.

The verification screen accepts one six-digit field with OS one-time-code autofill and paste sanitization, enforces a persistent 30-second resend cooldown, maps incorrect/expired/throttled Clerk responses to safe recovery text, and can replace the signup email before sending a fresh code. It accepts completion only from Clerk's returned `complete` status plus created session/user IDs, persists no code or session token, and activates that session before invoking the protected bridge. A verified-completion marker bound to the Clerk user makes session activation and DOB completion resumable after process recreation; the root redirects only the matching signed-in user back to this retry state.

Auth presentation lives in `features/auth/components`: device light/dark palette independent of saved account themes, system-font controls, 58-point fields, trailing-arrow capsule actions, compact login labels/placeholders, no form hero, and six animated progress dots in the signup toolbar. Forms scroll only when content exceeds available height (including keyboard/large-text cases). The shared UI package exports `useSafeAreaInsets` alongside its provider to avoid duplicate pnpm context instances in the DOB modal. Existing mobile password reset, verification/resend, and authenticated DOB recovery remain wired; SwiftUI's unavailable reset action is not copied. No additional backend reads/writes or indexes are introduced at 1M+ DAU. Native splash changes require rebuilt binaries; isolated generation checks enforce dark iOS assets and Android night resources.

The production `/login` route uses Clerk password sign-in for username or email, preserves password bytes exactly in volatile component state, suppresses identity enumeration in rejected-credential copy, and locks concurrent submissions. A completed attempt is accepted only with Clerk's created session ID; the route activates that exact session and returns through the canonical root bootstrap gate. If Clerk later returns `needs_second_factor` or `needs_client_trust`, the client prepares only a supported email-code factor, keeps the six-digit code in memory, applies a 30-second resend cooldown, and requires a completed created session before activation. Unsupported factor states fail explicitly. The checked native environment currently enables password with no second factor; Apple and Google require their separate native-provider configuration and are not represented as working buttons.

The password recovery routes live under `/password/forgot`, `/password/verify`, `/password/reset`, and `/password/complete` in `features/auth/password`. They use Clerk's environment-confirmed `reset_password_email_code` first factor, the device-adaptive shared auth scaffold, six-digit code sanitization, resend cooldowns, password confirmation rules, and safe completion routing. Email/safe target context may travel as non-secret route state; reset codes and new passwords stay in component memory only. When Clerk returns a created session, the route activates it and sends the user through root bootstrap; if Clerk completes without a session, the completion screen returns to Login. This adds no 35mm API route, DB/cache/queue/worker behavior, schema, migration, or index because Clerk owns reset delivery, expiry, throttling, and password policy.

The post-verification bridge performs protected `/v1/me`, naturally idempotent `PATCH /v1/profiles/me`, and `/v1/me/onboarding-status` operations in sequence and runtime-validates that the owner response contains the requested DOB before signup may advance. Failure preserves the DOB and exposes an explicit duplicate-safe completion retry; success clears the signup draft and returns through the root onboarding-status router. The API's existing owner-only profile projection and `profileWriteRateLimit` remain authoritative. Shared `@35mm/validators/date-of-birth` validates real calendar dates and future-date rejection in mobile and API code; public profile projections continue returning `dateOfBirth: null`. Minimum-age and regional eligibility remain blocked on product/legal policy and require server enforcement before public signup release.

These signup slices add Clerk account creation plus one authenticated profile set operation but no API route, database schema, Redis/cache, queue/worker job, UGC lifecycle, or index. Each stable locally valid username candidate creates at most one client request; the existing endpoint performs two parallel indexed point lookups against unique `profiles.username` and primary-key `username_locks.username`. After one successful verification, completion performs two bounded owner reads and one naturally idempotent, rate-limited update against the existing unique `profiles.user_id` path. At 1M+ DAU, this work scales with new-account completions rather than active-user reads, creates no N+1 or live-count path, and requires no new database index.

Phase 1.6 adds private package `@35mm/mobile-ui` as the React Native adapter over `@35mm/design-tokens`. It owns a fail-loud controlled theme context; local DM Serif Display, DM Sans, and DM Mono asset aliases; one Lucide/`react-native-svg` icon map; safe-area screens; text, card, divider, button, icon-button, badge, chip, counter, field, controlled password field, and avatar primitives; centered/full-screen modal and confirmation surfaces; the web/Swift-aligned 32-point action sheet with 58-point rows, 80-point drag dismissal, Android-back handling, and Reanimated/Gesture Handler UI-thread motion; a provider-scoped four-item toast queue; bounded pulse skeleton primitives; and explicit loading, empty, offline, unauthorized, private, deleted, permission-denied, pagination, and inline-notice states. Password visibility actions expose state-specific accessible labels and retain the 44-point icon-button target. Reduce Motion removes autonomous spatial motion, skeleton geometry is hidden from assistive technology, interactive targets meet the 44-point floor, and sheet/modal surfaces declare modal accessibility containment. Phase 1.7 now loads its fonts and composes its safe-area, controlled theme, and toast providers at the app root.

`@35mm/mobile-ui` is presentation-only. Its toast queue and skeleton line counts are bounded in client memory, and it performs no API request, database read/write, cache operation, queue/worker job, schema change, or server mutation. At 1M+ DAU it adds no backend volume and requires no database index. `react-native-gesture-handler`, Reanimated 4.5, Worklets 0.10, and `react-native-svg` are native dependencies, so this slice is binary-affecting and is not eligible for JavaScript-only OTA delivery; isolated development/preview CNG checks cover both generated platforms.

Phase 1.7 composes the root error boundary, Clerk Expo provider and supported SecureStore token cache, account-gated React Query persistence, AppState/NetInfo lifecycle bridge, injected API client, and shared UI bootstrap. React Query owns server state; Zustand owns transient drawer/chrome/composer presentation and persists only validated theme preference. One explicit accessible 35mm loading surface covers Clerk, query-scope, and font initialization; theme preference restores asynchronously without blocking route rendering. Query persistence is opt-in, successful/idle, explicitly non-sensitive and bounded, limited to 32 entries/256 KiB each/1 MiB total/six hours, and keyed by a SHA-256-derived account scope; the prior account cache is removed before a new scope renders. Runtime configuration requires explicit `EXPO_PUBLIC_API_URL` and `EXPO_PUBLIC_CLERK_PUBLISHABLE_KEY`; preview requires a non-loopback HTTPS API origin.

Phase 1.8 introduced a deterministic internal foundation gallery, now retained at explicit route `/quality/foundation` after the production root became the Phase 2 auth gate. It renders shared controls plus loading/offline/error states in explicit light/dark and Reduce Motion conditions, with stable accessibility/test identifiers. React Native Testing Library verifies headers, images, fields, tabs, selected/disabled state, state transitions, and the 44-point interaction floor. Maestro smoke and screenshot flows inject only approved internal app IDs and open the quality route explicitly; screenshot capture crops to the 35mm-owned canvas. A `pngjs` comparator enforces equal geometry, bounded channel noise, and a 0.1% changed-pixel ceiling against reviewed fixed iPhone 15/iOS 17.5 and Pixel 6/API 36 baselines. Missing baselines fail rather than self-approve.

The performance harness accepts only named-device, release-build evidence with commit, tool/version, Hermes bundle size, and at least five samples for cold/warm launch, steady memory, and slow-frame percentage; it reports p50/p95 and does not invent thresholds before physical-device measurement. The local host now has Xcode 26.6, one valid Apple Development identity, CocoaPods, JDK 17, Android Studio/SDK/ADB/emulator, Maestro, and EAS CLI. An Android development binary builds and installs on the Pixel 6/API 36 AVD, its Metro bundle resolves, and the development-client Maestro smoke flow passes. Expo's development Tools overlay occupies the top-right corner, so the internal gallery keeps its theme control clear of that development-only surface; preview behavior is unchanged. The ignored development environment contains Clerk plus a health-checked LAN API origin, which must be refreshed if the workstation address changes. EAS authentication is optional while local builds are used. The Personal-Team-signed iOS Debug app builds and installs on the connected/trusted iPhone 13 Pro, but it fails before Expo launcher UI with a null React Native bundle URL; no Continue or Local Network prompt is present, and earlier instructions claiming otherwise were incorrect. Physical-iOS smoke validation therefore uses the development identity in Release configuration with an embedded Hermes bundle. Device syslog and LLDB proved the first Release black screen was not a surviving bootstrap wait: Release dead-code stripping removed the generated Expo provider, so `ExpoAsset` and Expo Constants never registered even though React owned a root view. After provider retention, device logs exposed a missing embedded manifest because Expo's shell script split the space-containing project path. The corrected app now retains the provider, embeds the valid Expo Constants config, visibly renders the Foundation gallery, remains alive, and has clean startup logs without prior fatal signatures. Explicit bootstrap loading remains as fail-visible protection. Maestro 2.7.0 does not support local physical-iOS execution. No representative low/mid-range Android device is available; reviewed fixed-profile baselines and release performance evidence remain unclaimed. Harness code is fixed-size local/test infrastructure: it creates no API, DB, Redis, cache, queue, worker, schema, or index impact at 1M+ DAU.

Private package `@35mm/api-client` contains no React, React Native, storage, or Clerk dependency. The app injects the API origin, Clerk token provider, fetch implementation, request-ID generator, abort signal, and platform metadata. It provides bounded read/mutation/upload timeouts, one-megabyte response protection, standard API error decoding, redacted diagnostics, cancellation, optional runtime parsing, and at most three retries; non-GET retry requires an idempotency key. The provider foundation introduces no automatic endpoint call, API route, DB/Redis/worker operation, schema, or index. At 1M+ DAU, network volume remains feature-driven and unchanged; retry and device-cache work is explicitly bounded.

Phase 1.2 adds Jest 29.7 through `jest-expo` 57.0.5 and React Native Testing Library 14.0.1. Tests live outside Expo Router's route directory, and Jest discovery is rooted at `src` so generated-native and local Expo cache artifacts cannot create unbounded enumeration work. `pnpm mobile:check` is the deterministic CI entry point and runs the design-token, API-client, and mobile-UI package checks, strict mobile TypeScript, Expo ESLint, public Expo config validation, quality-harness contracts, native policy/generation checks, and serialized Jest CI coverage. Current tests cover 29 source-backed token invariants, 6 API transport cases, and 105 app/UI cases, including Login/session challenges, signed video response trust boundaries, plus bounded and resumed TUS transfer.
...
- Web Edit Profile updates display metadata through `/v1/profiles/me` and routes username changes through `/v1/me/settings/profile`, preserving the durable Clerk-first reservation/finalization flow and its rate limit. Role changes write both role and headline fields so profile and post bylines stay aligned; an explicit `null` headline context is a valid clear when the selected role is cinephile, while non-empty cinephile context remains invalid.
...
- Viewer-specific public profile detail, follower/following lists, and authenticated `/v1/me` responses use `private, no-store`. Followers and Following are tabs inside one fixed-height web Connections modal; owner views add Requests as a third tab, and each connection list keeps its independent cursor/query cache while switching. Each connection-list page resolves viewer relationship state through one indexed join and returns server-authoritative `viewerOwnsProfile`, letting authenticated follower rows expose Follow, Follow back, Requested, or Unfollow without trusting transient page ownership state or issuing per-row reads. Unfollowing a mutual follower keeps that follower row visible, changes its relationship action to Follow back, removes the target from the viewer's cached Following list, and updates actor/target profile counters only when the idempotent API reports a deleted relationship. Web profile detail revalidates whenever mounted, while accepted suggestion/modal follows update the actor's cached `followingCount`, the target's cached `followerCount`, and both connection lists before bounded invalidation. Private-account requests expose `requested` state and do not change either count until acceptance.
...
- Theme and accent color. Theme values currently accepted by settings are `auto`, `light`, `dark`, `letterboxd`, `matinee`, `matrix`, `oppenheimer-bw`, and `barbie`.
- Media playback preferences: video autoplay, default quality, captions default, caption display style, and quiet mode.
- Discover streaming-service preferences: an ordered, bounded array of shared logical service IDs. User-defined order is preserved through validation, storage, and the Discover filter shelf. The web maps these IDs to TMDB provider IDs; the catalog is fixed-size, so no index or pagination is required.
...
- `watched_on` nullable date and `is_rewatch` boolean store film diary details independently of publication time. Older entries retain an unknown watch date; diary ordering falls back to their UTC creation date.
- Nullable `creation_key` UUID and `creation_request_hash` provide per-author create idempotency. A partial unique `(user_id, creation_key)` index excludes entries without keys. Reusing a key with changed content or after soft deletion returns a conflict.
- The partial diary index covers `(user_id, coalesce(watched_on, UTC creation date) DESC, created_at DESC, id DESC)` for non-deleted, non-repost logs/reviews. Date fields and indexes are introduced by migration `0065_film_diary_watch_details`.
...
- Denormalized `like_count`, `comment_count`, `repost_count`, `bookmark_count`, `quote_count`.
...
- Type enum also includes `film_logged`, `report_status_update`, `content_moderated`, and `content_under_review`. Legacy `chat_reaction` rows remain schema-readable, but main notification writes, reads, mutations, email, and realtime reject them because chat owns its activity channel.
...
- Partial `(like_count, id)` and `(created_at, id)` indexes cover popular/recent public custom-list browsing without scanning private, deleted, or watchlist rows.
...
- `GET /v1/profiles/:username/stats`
  - accepts an optional validated `year` query and returns available activity years plus real stats for visible logged films: runtime hours, runtime coverage, unique-film and rewatch counts, rating distribution, review counts/likes, favorite and most-watched films, release decades, genres, credited directors/cast/music people, countries, languages, and period activity
  - runtime hours divide stored minutes by 60 and use active `catalog_titles.runtime_minutes` as fallback; TMDB film resolution hydrates missing runtime/language/country metadata through the existing cached, rate-limited detail proxy
...
- `GET /v1/feed/profiles/:username/posts`
  - signed-out viewers get a teaser only: `limit` is capped at `PROFILE_FEED_GUEST_POST_LIMIT` (5), `nextCursor` is always `null`, and any `cursor` param returns `401 UNAUTHORIZED`; `hasMore` stays truthful so clients can render a sign-up gate. Lists, diary, and stats reads are unaffected. Guest cache keys use the capped limit.
...
- `GET /v1/lists` (public custom lists; `popular | recent`; bounded cursor pages)
...
- `PATCH /v1/me/settings/streaming-services`
...
- `GET /v1/catalog/titles/by-slug/:slug`
...
- `GET /v1/catalog/people/by-slug/:slug`
...
- `middleware.ts`: public route definitions, guest-only auth page redirects, and Clerk protection. Beyond static/auth/api allowlists, `isPublicContentPath()` opens read-only content pages to signed-out visitors: profiles (`/:username`), profile tabs, post detail (`/:username/post/*`), discover, films, lists, list detail, `/film`, `/tv`, `/title`, `/person`, `/company`, and allowlisted `/:role/:person-slug`. App sections (`feed`/root-authed, `chat`, `bookmarks`, `drafts`, `new`, `notifications`, `settings`, `70mm`, `suggestions`, `contribute`, `festivals`, `communities`, `onboarding`) stay behind `auth.protect()`. The corresponding read APIs already use `getOptionalAuthUser`, so guest reads carry no token.
- `app/layout.tsx`: global providers and pathname-aware scroll ownership. Each pathname stores and restores its own bounded session scroll position, unvisited pages start at the top, and explicit post-detail returns still restore only the recorded source feed path with anchor support.
- `app/(shell)/layout.tsx`: authenticated shell, auth bootstrap, onboarding gate, and shared layout grid.
...
- At `xl`, the home shell uses its previously reserved left rail for `ProfileCompletionWidget`. Signed-in users with missing avatar, cover, bio, or location see a restrained four-detail progress card; completed details remain visible as subdued confirmation, and fully completed profiles render no widget. Signed-out guests do not mount the rail. Each missing row deep-links to the owner profile with a validated `editProfile` target. The existing edit dialog opens, scrolls/focuses the requested control, and briefly highlights it. The dialog includes the existing cover upload/crop flow in a compact editor variant, so all four completion actions share one accessible editing surface. `GET /v1/me` returns four completion booleans computed from profile columns in its unique `user_id` lookup; the shell reuses that React Query record and adds no home-page request. Existing profile/media mutations and their rate limits remain authoritative. At 1M+ DAU this adds constant response bytes and no query, N+1 path, cache, queue job, schema change, or index.
...
- `(shell)`: product routes. Most require auth, but the content pages listed under `middleware.ts` above render for signed-out visitors too. `ShellGrid` wraps its subtree in `AuthPromptProvider` (`features/auth/components/AuthPromptProvider.tsx`): `useAuthPrompt()` exposes `promptLogin({ mode?, title?, message?, next? })` and `requireAuth(action, opts)`, and a shared `AuthModal` (`features/auth/components/AuthModal.tsx`) renders the real Clerk forms in place: a two-column centered dialog (decor column with bundled `public/landing` artwork, brand mark, and per-mode or caller-supplied headline/message; form column rendering `LoginForm`, `SignupForm`, or `VerifyEmailForm` one at a time so the dialog height follows the visible form) that collapses to a single form column below 768px. Mode switches (login ↔ signup) and the post-signup email-code step happen inside the modal without navigation; the `next` path is passed as a prop (unsafe/non-relative values are dropped) and used by the forms' `completeSessionNavigation` on success. The standalone `/login`, `/signup`, and `/verify` pages keep using the same form components with `?next=`/`?email=` search params. When signed out, `SiteHeader` swaps the notifications/new-post/profile cluster for Log in / Sign up links that open the modal in `login`/`signup` mode (still `href`'d to `/login`/`/signup` for no-JS fallback) and gates the Feed/70mm/Chat/Profile nav items through the prompt (Discover stays a link); `MobileTabBar` and `MobileHeader` gate their equivalent nav for guests. For guests, `HomeProfileCompletionSidebar` does not mount and `HomeSuggestionsSidebar` hides people suggestions and the Letterboxd import card. For all viewers, the home/profile rail omits an empty `LobbyRoomsWidget` (`hideWhenEmpty`); signed-in people suggestions also disappear when loading finishes with no results. On title pages, `TitleActionButtons` (mark watched, watchlist) and the review composer trigger in `TitlePageView` call the prompt instead of mutating when signed out. Public profile, post, comment, poll, and list account actions (follow/request, message, mute, block, report, like, repost, bookmark, quote, reply, vote, list like/clone/create) also go through `requireAuth` so guests never mutate; share and other read-only controls stay available. Profile, post, and comment overflow menus hide signed-in actions for guests and keep only Share. Guest profiles never expose the full post/repost timeline: `ProfileTabContent` passes `ProfileGuestFeedGate` as `InfinitePostList`'s `guestFooter`, which replaces the infinite-scroll trigger for signed-out viewers with faded post skeletons and a "See {name}'s full profile" / "Continue to 35mm" banner that opens the auth modal in signup mode. The same headline/CTA is mirrored in a fixed bottom banner (portaled to `body`, measured to the feed column's left/width via `ResizeObserver`, faded with a gradient like X) that stays visible while the in-feed gate is off-screen and hides via `IntersectionObserver` once it scrolls into view. The API enforces the same cap server-side (see `GET /v1/feed/profiles/:username/posts`). Lists, diary, and stats tabs remain fully readable for guests.
...
- `/`: session-aware root. Signed-out visitors receive a restrained, single-field cinematic landing surface:
  ...
  verification request when Clerk challenges a new browser, with no 35mm API, database, cache, queue, schema,
  or index impact. No simulated feed, engagement
...
- `/lists`: standalone public-list discovery with popular/recent cursor streams and poster-stack cards; Discover, Lists, and Contribute share the sticky below-header section navigation.
- `/discover`: discovery.
...
- `/film/:slug`, `/tv/:slug`: public movie and TV title detail. Legacy `/title/:media/:id` URLs permanently redirect after resolving title metadata.
- `/:role/:slug`: clean public person filmography, with allowlisted professional roots such as `/actor/:slug`, `/director/:slug`, `/producer/:slug`, `/stunts/:slug`, `/lighting/:slug`, `/sets-decoration/:slug`, and `/special-effects/:slug`. Canonical URLs contain the normalized display name only; for example, `/writer/mark-gibson`. No canonical `/person` page exists. Specialist roles use the direct `app/(shell)/[username]/[slug]` route because the existing username namespace owns the root dynamic segment; `ProfileUsernameLayout` explicitly passes every allowlisted two-segment role path through to person content while retaining normal one-segment profiles and fixed profile tabs. Legacy `/person/:slug`, `/person/:slug/:role`, query-based role, and numeric person URLs permanently redirect to clean role/name routes.
- `/70mm`, `/70mm/upload`, `/70mm/:id`: 70mm surfaces (community-uploaded films, shorts, web series).
...
- `features/audio-rooms`: `The Lobby` right-rail presentation contract and empty state. The home/profile rail skips the empty lobby for all viewers via `hideWhenEmpty`; a populated room list can still render. A future production room service can supply bounded summaries and an open-room handler without changing the widget layout; realtime audio/signaling remains out of scope and unwired.
...
- `features/discover`: editorial TMDB-backed browsing with hero/aisle presentation and semantic theme tokens. `/discover` has no catalog search or filter controls; those belong to `/films`. Its streaming shelf reads the account's bounded saved-service lineup as one combined provider request without exposing session filter pills or an inline settings editor.
- Direct role routes provide clean-slug, TMDB-backed cast/crew filmographies cached by Next revalidation and used for metadata/display only. Resolution combines the normalized name with route role rather than choosing the first TMDB result. A unique exact-name result resolves directly. For duplicate exact names, the resolver checks each candidate's one-day-cached movie/TV credits in parallel and accepts only the one whose actual credits contain the requested role. This preserves one person while switching between clean role URLs even when TMDB's single `known_for_department` differs from the selected role. Same-name candidates sharing the requested role return no match instead of opening the wrong profile. Candidate fan-out is capped at the TMDB search-page maximum of 20; non-JSON, partial, or failed candidate responses fail the complete resolution closed. Cast, title links, filmography grouping, dropdown destinations, and duplicate qualification all use the same `personRoleForCredit` classifier. Regression coverage supplies a representative TMDB credit for every allowlisted role: actor, creator, director, writer, producer, editor, cinematographer, composer, production designer, art director, sets decoration, costume, makeup, visual effects, special effects, stunts, lighting, sound, casting, choreography, animation, camera, art, production, costume/makeup fallback, and crew fallback. A title remains independent across each contributed role and multiple equivalent jobs merge within that role. `PERSON_PAGE_USE_TWO_COLUMN_LAYOUT` switches between a wide profile-header layout and a compact poster grid beside a profile sidebar. Role selection changes canonical path and drives page headline and metadata title. Old `/person` paths are redirect-only compatibility routes. URL-backed filters narrow bounded TMDB credit data by media format, decade, and genre and sort it without additional requests.
- `app/(shell)/company/[id]`: TMDB-backed studio/network page for title-page production-company links. Cached with the same Next revalidation window as person pages. Reads company detail plus one bounded Discover movie page and one TV page (`with_companies`, popularity desc), merges/dedupes in memory, and shows at most 48 posters. No catalog DB, Redis, or worker path.
- `features/title`: title pages. Cast links use `/actor/:slug`; hero and About-tab creator, director, writer, producer, and detailed crew names use clean role-specific roots; studios use `/company/:id`.
...
- `features/70mm` and `features/videos`: real Bunny upload/playback modules coexist with legacy mock discovery shelves. Festivals and communities remain future or mock-heavy surfaces.
...
Film logging (2026-09-13): the web composer presents film, optional half-star rating, watched date, optional coarse watch venue, optional review, and an explicit feed-sharing choice. Any visible review text creates a review; an empty body creates a log, regardless of entry point or character count. The native range input supports keyboard half-star steps and an explicit unrated state, using the same green rating tokens as title-page stars. New entries default to the user's local calendar date and public profile visibility; unchecking Share to feed skips follower-feed fanout but leaves the entry public on the profile. The last successfully saved feed-sharing choice is scoped to the signed-in account in browser storage; storage failures keep a profile-only default and surface a notice. Editing preserves existing visibility and restores rating, date, venue, and rewatch. `PATCH` records these values in `post_edits` with the previous type, film, body, and visibility; identical retries do not add edit history.

`GET /v1/feed/profiles/:username/posts?kind=diary` filters logs/reviews before pagination and orders by watched date, then exact creation timestamp and ID. The cursor retains PostgreSQL microseconds. Public diaries paginate under existing visibility, profile, moderation, block, and mute rules; private entries remain owner-only. Diary rendering uses date-only values without browser timezone shifts, and profile stats use the same effective watch date plus bounded venue distribution. Existing cache invalidation remains in place; the shared feed namespace is `feed-cache:v6` and profile stats use `profile-stats:v5`.

Log submission captures one immutable payload and UUID for an unchanged draft's retries. The API inserts at most one post and one set of durable counter/fanout intents per author/key, then can recover idempotent post-commit work on replay. Mention delivery uses the existing unique notification source key to reuse the same inbox row. At 1M+ DAU, each save remains one bounded authenticated, rate-limited mutation with asynchronous counter updates; diary reads fetch at most `limit + 1` indexed rows and use existing batched hydration. Sharing preferences add no server request, and there are no new live counts or follower-wide synchronous writes.
...
The desktop composer modal sits 12% of the viewport below the top edge, sizes to its content, and caps growth at `min(680px, 80dvh)`. Its mode header and formatting/action toolbar remain outside the single internal scroll region, so growing text, polls, media, link previews, and quoted posts cannot expand the dialog beyond that cap or move submission controls offscreen. Quote open copies an in-memory snapshot from the already-loaded source card — identity, body, native image/video media (including Bunny `videoAssetId` and stored width/height), and YouTube/Vimeo link-preview metadata — so the composer preview can render the same bounded quoted media grid as feed `PostCardQuoteEmbed` without a follow-up post read. A quote is always a regular post, so the composer hides the mode tabs while a quoted source is attached and drops the leftover tab-header gap. A lone quoted video keeps the source aspect ratio through `initialAspectRatio` plus Bunny playback width/height instead of a forced 16:9 cell. This is client-only layout/snapshot behavior: it adds no API read, database write, cache entry, queue job, schema change, or index, and therefore has no request-volume impact at 1M+ DAU.
...
- Quote posts are normal authored posts (`text` or `image`) whose `quoted_post_id` points to the canonical source. Discussion, log, and review types cannot carry a quote; `createPostSchema` rejects those combinations and the web composer hides the mode tabs while quoting. `createPostSchema` validates the UUID and the API re-authorizes/canonicalizes the source server-side before insert. Creating a non-repost quote writes `posts.quoteCount` +1 on the source in the same counter outbox transaction; soft-deleting that quote writes -1. `quote_count` is denormalized and overlaid with pending `counter_job_deltas` on feed/detail reads—no live `COUNT()` on the hot path. Feed/detail/profile/bookmark responses expose `quoteCount` plus a bounded, non-recursive `quotedPost` preview (original author, rich text, media, film, link preview, and poll) plus `quotedPostUnavailable`; web and iOS render it inside the quote card and use a tombstone when the source is deleted or inaccessible. Web exposes `/:username/post/:postId/quotes` from the repost menu only when `quoteCount > 0`. Its feed-width page calls rate-limited `GET /v1/feed/posts/:postId/quotes`, applies source and quote visibility/moderation/block/mute policy, excludes simple repost activity, and supports `latest` or `top` (most liked) cursor order. Latest uses `posts_quoted_post_id_created_at_id_idx`; top uses partial `posts_quoted_post_id_like_count_created_at_id_idx`. Each request remains one bounded page plus existing batched hydration, so viral quote threads scale with page size rather than total quote count at 1M+ DAU. Quote create/delete reuse the existing post mutation, rate limit, and `counter.outbox` path; reconciliation repairs `quote_count` from non-deleted, non-repost rows with that `quoted_post_id`.
- Repost and quote sources for a page share one bounded primary-key `IN` query, followed by the existing batched poll and rich-mention hydration; there is no per-post/N+1 read. Source access checks cover post visibility, deletion/moderation, original-profile privacy/moderation, blocks, and mutes. Cached quote previews are rechecked against moderation read status and tombstoned when access is revoked; cached aggregated repost proof rechecks every named actor and removes hidden identities while preserving a separately visible original. Feed/profile response caches use the shared `FEED_CACHE_NAMESPACE` (`feed-cache:v6`) constant in API and worker so writes and invalidations cannot drift across namespaces.
...
- `counter.increment` is implemented for post like/comment/repost/bookmark/quote counters, comment likes, poll totals/options, profile films/post/follower/following counters, and film list like/comment/entry counters. Post create/delete and repost create/delete write the profile `postCount` delta in the same transaction as the post mutation. Creating or deleting a non-repost quote also writes the source post `quoteCount` delta in that same transaction. Creating or deleting a canonical-film `log`/`review`, plus editing one across the film-attached boundary, writes the matching `filmsLoggedCount` delta in that same transaction. Reposts never count as diary logs or quotes. Profile detail and `/v1/me` overlay pending profile film/follow deltas for read-after-write consistency while BullMQ drains the durable outbox; film-list detail, watchlist, profile-list, film-list, and public-list reads apply the same indexed pending-delta overlay in one bounded query per response. Web diary and list mutations invalidate their active query keys so refreshed UI consumes the overlay immediately.
...
Chat activity has a strict channel boundary: reactions update only chat inbox metadata, chat unread counters, and chat realtime events. Main notification creation rejects `chat_reaction`; main list/count/read mutations filter legacy rows; worker email/realtime publishing and web realtime parsing reject them as defense in depth.

Client path:

- Web notifications use React Query for list/read/unread state.
- Native iOS notifications keep cursor paging and idempotent read/unread mutations in `NotificationsViewModel`; the activity tab mounts a reusable equal-width `HeaderTabBar` inside the app header for All/Unread filtering, and the standalone notifications destination reuses the same filter bar above the list. The body uses a native page-style `TabView` pager so horizontal swipes track the finger in real time and header taps animate through the same page model. Each filter owns a bounded cursor stream with idempotent initial loading, so normal swipes do not re-fetch; read-state mutations refresh the sibling filter after user action so All/Unread stay consistent without unbounded reads. Row presentation is split across `NotificationRow`, `NotificationAvatarStack`, `NotificationContentPreview`, and `NotificationThumbnail`. Rows use a compact actor/action/time sentence, render already-resolved film/post copy without a filled container, keep poster artwork on the trailing side, and use full-width hairline separators. Each row exposes an independent 44-point more-options control through the shared `BottomActionSheet`, with only existing post navigation and read/unread mutations. Avatar/poster delivery uses Kingfisher, and bundled actor wording is derived in bounded client memory from the API's capped actor profile slice plus denormalized `bundleCount`; this adds no API, database, cache, queue, or index work.
...
- Incoming follow requests use the dedicated `/v1/follows/requests/received` data source. The notification screen always renders a compact Follow requests gateway above the activity feed, including a zero-pending state, and fetches at most two profiles plus the denormalized total; selecting it opens `FollowRequestsView`, whose dedicated view model pages 24 requests at a time with the existing opaque cursor and applies accept/decline optimistically with rollback on failure. The mutations reuse the existing rate-limited, idempotent follow endpoints, and the per-viewer reads remain bounded and scale linearly without new schema, cache, queue, or index work.
...
Web post composer accepts one MP4 or WebM video up to the API's 120 MB limit, renders the local object URL in an HTML5 player before and during publish, and reports byte progress from the direct presigned PUT. Video transfer therefore bypasses API compute and memory at 1M+ DAU; the API handles only the existing rate-limited presign and bounded post mutation. No new database query, cache, worker job, or index is required. The current R2 original remains the stored media URL until a dedicated Bunny.net, Cloudflare Stream, or AWS video pipeline is selected.
...
- Namespace: `profile-stats:v5`; cache keys include selected year.
...
- Author post create/edit/delete, post-owner interaction invalidation, and profile edits clear every author/year stats key through the author index set. Profile stats queries use `posts_user_type_created_at_idx` for per-user log/review scans; credit lookup uses `catalog_titles_legacy_film_idx` and `catalog_credits_title_department_idx`; top film, watch venue, and metadata rankings are bounded. No new index is required.
...
- Native iOS Discover uses the same server-side TMDB proxy and default shelf composition as web, then resolves selections through the canonical catalog API before native title navigation. Its root tab uses the same header tab bar pattern as Notifications with Discover, Lists, and Films sections; Lists and Films use existing bounded cursor endpoints (`/v1/lists` and `/v1/catalog/titles`) without adding backend routes. Web title and composer paths still rely on TMDB proxy or local/static data in places.
...
- Web `/films` provides one paginated catalog surface over 35mm movie records
  plus TMDB fallback. Its type filter supports all titles, movies, TV shows,
  keyword-classified web series, short films, documentaries, and miniseries.
  TMDB movie/TV identities remain distinct during merge and deduplication.
  Popularity is default ordering; matching 35mm records retain canonical IDs
  in their TMDB-ranked positions instead of receiving source priority. Later
  pages append without re-sorting rendered cards and load only through the
  explicit catalog control, preventing scroll-anchor fetch loops.
...
- Move 35mm-native TV/web-series browsing from TMDB fallback onto normalized
  `catalog_titles`; legacy `films` rows currently represent movies only.
...
- `apps/ios/ThirtyFiveMM/Features/Intro/IntroView.swift` is the signed-out entry surface. It presents a Pinterest-inspired three-column wall of nine bundled movie posters in `WelcomeHeroView.swift`, scrolling seamlessly in repeating columns beneath a bare top wordmark, with a white bottom fade and “Your cinema. Your people.” / “The social network for all things cinema.” copy. Black Sign up and light-gray Log in capsules retain the existing Clerk-backed destinations, with fixed Terms/Privacy links beneath. The light welcome layout stays fixed at standard text sizes and permits scrolling for enlarged text or compact landscape height. Poster animation runs at a bounded 30 Hz only while visible and active, and becomes static with Reduce Motion. The flexible artwork fills available height so legal copy sits just above the bottom safe area. Poster assets reuse the curated web landing selection (provenance in `Resources/WelcomePosters.md`); no runtime image fetch or authenticated API read occurs before account navigation. `RootView` renders a launch-screen-matching wordmark handoff during Clerk/API bootstrap, without a visible progress spinner or duplicate branded loading screen. `AuthManager` does not show signed-out UI until Clerk has finished loading; if Clerk restore times out, the app stays on retryable session recovery instead of clearing into Intro. `SignInView` and `SignUpView` are prompt-first, top-aligned auth forms without the retained poster hero on the form screens: signup uses progress dots plus one requested field per step (name, username, email, password, DOB, verification); login uses email-or-username plus password with password reveal. Those form screens intentionally avoid beige/brown palettes, decorative film-card art, and rounded/serif display fonts; they use plain white backgrounds, black/gray fields, and standard SF system typography in the Spotify/Pinterest/Instagram reference pattern. `AuthManager` persists DOB after Clerk session activation through the existing owner-only profile update endpoint before entering onboarding. This adds no new API route, schema, cache, worker, or index; the profile write remains server-authorized and rate-limited by the existing profile mutation path.
...
- `apps/ios/ThirtyFiveMM/Core/Models/Catalog.swift`, `Features/Discover/*`, and `Features/Title/*` implement web-parity discovery over the shared cached TMDB proxy, canonical catalog resolution before native title navigation, title detail, cast/crew, review browsing, watchlist actions, and native action sheets. The retained SwiftUI Discover root has top header tabs for Discover, Lists, and Films using the same `HeaderTabBar` as Notifications; Lists reads public popular list cursor pages and Films reads catalog movie cursor pages with canonical title navigation.
...
- `apps/ios/ThirtyFiveMM/Features/Profile/*` implements mobile-web-aligned native profile identity, Posts/Reposts/Diary/Lists/Stats tabs, cursor paging, lazy repost/list/stats reads, optimistic post actions, relationship/moderation confirmation dialogs, and a validated edit-profile flow with avatar/cover upload and removal. Profile paging preserves the stable outer vertical profile scroll and mounts only the settled page plus, during a transition, one adjacent/selected destination overlay. The settled page alone owns intrinsic height; the destination overlay cannot mutate parent layout while dragging, and both pages are clipped to the pager viewport. `ProfilePagingPanGesture` installs one bounded `UIPanGestureRecognizer` on the hosting window and accepts it only when the touch began inside the pager, an adjacent tab exists, and horizontal velocity exceeds vertical velocity by 15%. Once horizontal intent is confirmed, `cancelsTouchesInView` cancels pending post taps; vertical-dominant movement fails the pager recognizer and stays with the outer scroll, vertical scroll recognizers may recognize simultaneously, nested horizontal shelves retain their own gesture priority, and the system edge-back gesture must fail before profile paging begins. Translation directly drives page offsets, the logical LTR/RTL-aware underline, label reveal/collapse, and subtle icon emphasis from one progress value. Settled active tabs always show icon plus label; inactive tabs show icons only. Drag and tap transitions interpolate outgoing and incoming chrome instead of snapping after page settlement. Distance/velocity projection selects settle versus cancel, Reduce Motion commits without spatial animation, and VoiceOver disables the pan recognizer while preserving fully labeled tab buttons. Destination data begins loading when a tap or adjacent drag first presents it, while view-model idempotency retains loaded payloads; only current/destination page trees exist, and active-gated `LazyVStack` pagination prevents preview pages from draining cursors. Reposts uses the existing profile feed route with `kind=reposts` and an independent bounded cursor. `ProfileHeaderView` matches the mobile web action hierarchy with 44-point circular share/overflow controls beside the overlapping avatar and a full-width follow/edit capsule below profile metadata. Profile avatar/cover previews capture the tapped media's global source frame, suppress system cover choreography, and animate the full-screen image outward from that source over a translucent black backdrop with footer/close chrome entering separately; Reduce Motion falls back to a short fade. Each lazy tab read remains an existing bounded 20-item indexed cursor request; no new unbounded query, synchronous write, cache, queue, rate-limit, soft-delete, API shape, database schema, or index is introduced at 1M+ DAU.
- `apps/ios/ThirtyFiveMMTests/ProfileFeatureTests.swift` covers profile/stats/list decoding, edit validation and explicit-null encoding, cursor deduplication, horizontal-intent arbitration, signed LTR/RTL drag progress, adjacent-page selection, distance/prediction settlement, lazy request deduplication, shared feed-renderer diffable reconfiguration under profile data, and optimistic rollback.
...
Both values are required by the Phase 1.7 root bootstrap. Preview accepts only a non-loopback HTTPS API origin. The Clerk value is a public publishable key; session tokens remain in Clerk's Expo SecureStore token cache and are never persisted in AsyncStorage or query state.
...
- Modal focus stack.
- Mobile root-layout/Router-bootstrap Jest tests, Expo Doctor, strict TypeScript, Expo ESLint/config resolution, CNG boundary/plugin/autolinking policy checks, isolated two-variant native prebuild, and iOS/Android Hermes bundle export.
...
pnpm --filter @35mm/mobile typecheck
pnpm --filter @35mm/mobile lint
pnpm --filter @35mm/mobile config:check
...
9. Define audited appeals, unsuspend, and unban actions if account reinstatement enters scope.
10. Continue React Native Phase 2 with auth process-death, offline, throttling, expiry, accessibility, and visual tests. Password Login, password recovery, email-code challenge handling, signup verification, Clerk session activation, and the authenticated DOB completion bridge are implemented. Apple/Google native provider flows remain unwired. Embedded-bundle iPhone build/sign/install, visible gallery rendering, sustained process, and clean startup logs are verified; representative physical Android evidence remains deferred by user and must still precede public release.

Post-V1 or gated surfaces:

- 70mm (community-uploaded films, shorts, web series).
- Communities.
- Festivals.
- Push notifications.
...
See [Bunny video integration](./bunny-video-uploads.md) for route contracts, quota and retention details, account configuration, operational limits, and the 1M-DAU capacity assumptions. React Native now consumes the post-video path; unrelated existing 70mm mock shelves are unchanged.

### React Native video-post client (2026-09-06)

Onboarded authenticated users in `apps/mobile` now enter a cursor-paged, FlashList-backed feed with signed Bunny post playback and a video composer. The client validates shared limits, previews locally with Expo Video, transfers directly to Bunny by reading fixed 8 MiB `expo-file-system` slices through TUS, persists only a bounded non-secret resume identity, acknowledges processing, and publishes with the existing idempotent post route. It supports visibility, upload cancellation/retry/progress, processing/failure presentation, optimistic like/repost/bookmark rollback, sharing, and owner soft-delete. Playback uses an HTTPS-validated five-minute embed grant in a WebView, follows account autoplay/start-sound and quiet-mode settings, limits automatic playback to one at-least-half-visible card, pauses in the background, and disables autoplay under Reduce Motion. Camera, microphone, background playback, PiP, and arbitrary upload/playback origins are not enabled.

This preserves the existing hybrid feed fan-out, cursor pagination, async counters, mutation rate limits/idempotency, post soft-delete, direct-provider media delivery, and leased BullMQ video reconciliation. At 1M DAU, the established 10,000 uploads/day and 20M playback authorizations/day assumptions are unchanged: API processes handle bounded indexed metadata requests while bytes bypass them, mobile transfer memory is bounded to one 8 MiB slice, and recycler rendering bounds mounted work. Existing `video_assets` primary/pending indexes cover the flow; no API route, schema, migration, worker job, or database index was added.

The same authenticated feed now renders the mixed `FeedPost` envelope through `features/feed/PostCard.tsx` instead of routing every item through a video-only card. Versioned TipTap text uses the focused `@35mm/validators/rich-text` contract and a bounded native renderer; cards cover image/video media, film attachments, link previews, read-only poll results, quotes, tombstones, counters, More actions, and owner soft-delete. Multi-image PostCard media can opt into the same 2+-image peek carousel through `EXPO_PUBLIC_POST_MEDIA_CAROUSEL`; the default remains the existing grid, and quoted-post previews remain grid-only. Non-control card taps and the comment action route to `/post/[postId]`; the route validates the canonical PostgreSQL UUID used by `posts.id` rather than the ULID-shaped identifier reserved for `films.id`. Detail loading reuses cached feed data when available, refreshes the authoritative post, and reads the existing flat comment endpoint in cursor pages of 20. Loaded pages are validated, deduplicated, and arranged into the server-enforced maximum three display levels.
...
ScrollRestore runs after route children in the root provider, keeps browser scroll
restoration manual, and restores the current pathname once, synchronously in a
layout effect before paint. Normal route navigation uses a bounded session map so
pages do not inherit another page's scroll position. If the URL commits before the feed,
...
### Public title and person slugs (2026-09-12)

- Public routes remain `/film/:slug`, `/tv/:slug`, and allowlisted `/:role/:person-slug`. Person identity is now a persisted catalog person plus TMDB external-ID mapping, not a role-dependent name search. Role switches and sharing retain the stored slug. Same-name people receive permanent numbered suffixes (`name`, `name-2`, ...), shared across every role. Existing mappings are never reassigned by the importer. Numeric compatibility links resolve exact IDs and permanently redirect; unenriched legacy payloads use that identity-safe redirect instead of guessing a name.
- `POST /v1/catalog/people/resolve` accepts a bounded TMDB source (movie, TV, person, or one search page), fetches provider metadata server-side, deduplicates by ID, and returns `CatalogPersonIdentity[]`. Public callers cannot supply names/slugs. The web TMDB proxy attaches stored slugs by exact credit ID before caching. Multi-search preserves mode, page, language, and adult filter. Unknown old bare links may bootstrap a search page once; stored slug ownership, never the requested role, decides subsequent resolution. Provider/catalog failures surface as errors rather than false not-found responses.
- The endpoint uses existing IP rate limiting (60/minute); web server requests forward the originating IP so users do not share one server quota. Cold imports recheck IDs under a transaction advisory lock, allocate numbered slugs, and batch both catalog-person and external-ID inserts. Retries return existing mappings. Existing unique slug and provider/external-ID indexes serve bounded lookups; no new index or migration. This follows catalog-first identity, with synchronous bounded metadata registration on cold misses (not counters or UGC writes). No UGC lifecycle, queue, or cursor-list endpoint changed.
- Scale assumption: 1M DAU × five title/person visits ≈ 5M/day. Enriched title/search payloads reuse existing CDN/Redis TMDB caches; hot profile slug lookups use the existing 45-second catalog cache and unique index. Imports issue a fixed number of batched queries, never one query per credit (maximum 2,000 source rows). Cold allocation serializes; launch/load testing must include cold-catalog throughput. Catalog writes invalidate the existing catalog cache; enriched upstream payloads retain their bounded existing TTL (24 hours for title detail, 12 hours for search). The importer treats slugs as immutable; administrative renames must preserve old links or purge upstream caches.
- API now requires `TMDB_API_KEY` for uncataloged source resolution, in addition to the web proxy's existing key. Local API configuration was populated from the existing web credential without changing its value. Deployment environments must configure the API key too. Tests cover ID-based attachment, duplicate-name isolation, role-independent resolution, retry rechecks, multi-search pagination, and outage handling. Architecture diagrams and chat docs are unaffected. Title/detail and multi-search requests carry a `person_slugs=1` response-version marker (not forwarded to TMDB) to bypass pre-change browser/CDN payloads. Verification: 433 web tests, 14 targeted API tests, web/API typechecks; Chrome confirmed Kevin’s directing filmography and Mark’s writer-to-director switch on clean URLs.

### 2026-09-21 — SwiftUI auth device appearance

Welcome, login, signup, verification, DOB picker, launch handoff, and session recovery use adaptive system background/label/field colors. Wordmarks render as templates and the poster fade matches the current surface. Primary actions are black with white text in light mode and white with black text in dark mode. Root appearance uses system automatic mode for loading/signed-out/recovery states without changing persisted account preferences; authenticated/onboarding states restore the account theme. No API, schema, index, or backend volume changes.

### 2026-09-21 — SwiftUI Clerk restore timeout recovery

Retained SwiftUI `AuthManager` now treats an unloaded Clerk SDK at startup as recoverable session restoration, not as confirmed signed-out state. A restart during device appearance changes can therefore show the splash/retry recovery while Clerk finishes or until the user retries, but it no longer routes to Intro solely because `clerk.session` was nil before `clerk.isLoaded`. Explicit Clerk sign-out and API unauthorized recovery still clear the session. No backend traffic, schema, index, cache, worker, or API contract changes.

### 2026-09-21 — Retained SwiftUI Discover header tabs

The retained SwiftUI Discover tab now wraps its content in the shared app header and `HeaderTabBar`, matching Notifications with Discover, Lists, and Films menu items. Discover keeps the existing TMDB-backed shelves and canonical catalog resolution. Lists reads existing public popular list cursor pages. Films reads existing catalog movie cursor pages and opens `TitleDetailView` by canonical catalog title ID. This adds no API route, schema, index, worker, cache, or mutation behavior; read volume stays bounded to existing cursor endpoints.


### Retained SwiftUI welcome navigation geometry (2026-09-21)

`IntroView` keeps viewport measurement inside the native `NavigationStack` root. Welcome ignores keyboard avoidance and clears inherited SwiftUI layout animation on its own subtree, leaving the navigation host responsible for the screen transition. It does not add outer safe-area padding or an oversized root frame; those caused a reported top-left fly-in on return and were removed. Enlarged-text/compact-height scrolling remains. This local presentation change adds no backend reads, writes, schema, index, or service dependency.

### 2026-09-21 — Native iOS splash appearance

The native iOS launch screen uses `Resources/LaunchScreen.storyboard`, selected by `UILaunchStoryboardName` and included in target resources. Its system background and appearance-aware `LaunchWordmark` render white/black in light mode and black/white in dark mode before SwiftUI starts. This matches the adaptive in-app launch handoff before account-theme restoration; the handoff keeps the wordmark/background only and deliberately avoids a visible progress spinner. Native launch-screen changes require a rebuilt, signed binary; the SwiftUI handoff update is source-only. Native launch screenshots were verified in both appearances on a separately identified simulator copy before this handoff cleanup; physical-device behavior remains unverified. No backend traffic or schema change.

### Retained SwiftUI Discover search (2026-09-24)

Discover now owns a capsule search field in its root header. Focus crossfades the avatar to Back, removes Messages, expands the field, and replaces browse content with recent queries/results while preserving mounted browse state. Reduce Motion disables the spatial animation. “See all” pushes native Recent searches with individual removal and Clear all.

`DiscoverSearchModel` persists only submitted/selected queries in account-scoped UserDefaults (50 entries, 200 characters per query), deduplicates case-insensitively, and clears visible state on account changes. History remains device-local, with no sync endpoint. Search uses existing canonical `/v1/catalog/titles` reads with 350ms debounce, cancellation/generation guards, 24-item cursor pages, explicit loading/error/retry states, and canonical title navigation. At 1M+ DAU, opening history adds zero remote reads/writes; typing issues at most one request per settled query and pagination is explicit. No schema/index, mutation, UGC lifecycle, cache, or worker changes.

Discover search follow-up (2026-09-24): the active header uses a 4pt leading inset and zero inter-control gap while retaining a 44pt Back target. The capsule background is an explicit focus button, including icon/padding hit regions; active text editing and Clear remain independent. Recent searches now uses app-owned Back/title/Clear all chrome with the system navigation bar hidden, matching Discover throughout push/pop and avoiding retained system-bar top clearance on refocus. The existing interactive-pop bridge preserves edge-swipe Back. Backend and history storage behavior are unchanged.

Discover history push coordination (2026-09-24): “See all” uses a native NavigationLink instead of a Boolean presentation mutation combined with explicit keyboard-focus clearing. Navigation owns keyboard dismissal; Discover no longer changes focus on disappearance. History retains its custom header and interactive Back.

Discover search session preservation (2026-09-24): account scope now comes from the shell's authenticated user ID rather than asynchronously loaded profile metadata. `useAccount` is idempotent and reports actual scope changes; only those changes reset search mode/focus. Reappearing after history therefore preserves the active search screen without waiting for keyboard focus to reactivate it. The inactive capsule uses a full foreground Button above the already-mounted TextField, removing disabled hit-testing as a prerequisite for the first tap. Repeated focus does not restart the search-mode animation. Existing per-account history bounds and search request behavior are unchanged.

Discover search title return (2026-09-24): search-result title destinations opt into `TitleDetailView` app-owned Back/title/actions chrome. The system navigation bar stays hidden across Discover → title → Discover, removing the visibility switch implicated in retained top clearance when search refocuses. The existing interactive-pop bridge preserves edge-swipe Back; title actions share the existing sheet. Other title entry points retain their current presentation. Local layout only: zero added backend reads/writes or indexes.
