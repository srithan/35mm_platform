...
- `TraditionalTabBarEnabled` gates the retained SwiftUI iOS bottom tab bar
  ...
  Every item keeps its accessible label, Home uses a feed/list-style stacked-card
  icon instead of a house, Profile renders the current user's avatar or initials
  fallback instead of a generic person icon, and Add uses an enlarged plus icon
  to preserve visual weight. The flagged
