...
The signed-out Phase 2 destination is a fixed-light Welcome screen aligned with the retained SwiftUI Intro reference. Its cinematic artwork is bundled locally; startup performs no remote-media or API read before an account action. The primary and secondary actions route to the planned signup-name and login paths, while Terms and Privacy can open only fixed 35mm HTTPS URLs and report failures accessibly. Those destination screens remain owned by later Phase 2 slices, so no fake route or success state is introduced. The deterministic foundation gallery also lives at internal route `/quality/foundation`; development and preview Maestro flows open that route explicitly instead of depending on the root destination. Until later onboarding and shell phases land, the internal gallery remains the bounded signed-in destination.

Phase 2 signup owns `/signup/name`, `/signup/email`, `/signup/password`, `/signup/dob`, and `/signup/verify` under `features/auth/signup`. All five routes use one fixed-light `SignupStepScaffold` for the cinematic hero, safe-area/keyboard handling, accessible five-step progress, typography, large-text wrapping, and back navigation where the state can safely move backward. The Name screen collects full/display name plus lowercase username, restores bounded non-secret fields from a versioned AsyncStorage draft, and never persists password, token, Clerk resource, or availability state. Mobile imports the canonical username contract through the focused runtime-safe `@35mm/validators/username` export. React Query debounces public `/v1/usernames/:username/available` reads by 450 milliseconds, passes cancellation through the injected API client, drops inactive typed-name queries immediately, rejects malformed responses, blocks continuation until the current username is confirmed available, and exposes connection/server failure as an explicit retry rather than success.
...
The Password screen adds production route `/signup/password` on the same scaffold. Shared `PasswordField` presentation supplies independent accessible show/hide state, secure entry, password-manager/autofill metadata, iOS password rules, and 44-point visibility actions. Feature validation preserves password bytes exactly, requires at least 8 characters plus exact confirmation, announces errors, and blocks duplicate continuation. Password and confirmation live only in the volatile portion of the existing Zustand signup store: back navigation retains them, AsyncStorage partialization excludes them, and process recreation requires re-entry. The current Clerk native environment was checked directly before implementation and enables password plus email-code signup with breach/strength checks; no environment response or secret is retained. Clerk remains authoritative at the later account-creation action, and this step routes to the separately owned DOB flow without creating an account.

The DOB screen presents locale-ordered Month/Day/Year numeric fields through one shared React Native surface, normalizes only a valid calendar value to `YYYY-MM-DD`, and rejects impossible or future dates without timezone conversion. Signup draft schema 4 persists bounded non-secret DOB, email-code send time, and verified-completion Clerk user ID while passwords and verification codes remain memory-only. The final DOB action creates or safely resumes the exact matching Clerk signup attempt, sends no DOB through Clerk metadata, clears password memory after account creation, prepares email-code verification, and retains DOB for explicit retry.
...
- `apps/ios/ThirtyFiveMM/Features/Intro/IntroView.swift` is the signed-out entry surface. It presents one primary account-creation route and one secondary sign-in route over an adaptive, locally rendered cinematic hero in `WelcomeHeroView.swift`; no image fetch or authenticated API read occurs before navigation into the existing Clerk-backed auth flow.
