...
- `TraditionalTabBarEnabled` gates the retained SwiftUI iOS bottom tab bar
  ...
  full-width native tab bar with Home, Discover, an icon-only Add action,
  Activity, and Profile; Add opens the composer and returns to the previous
  durable tab. The Add action keeps its accessible label while using an enlarged
  plus icon to replace the missing visible label. The flagged
